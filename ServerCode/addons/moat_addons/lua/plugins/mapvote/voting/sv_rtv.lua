...
function RTV.AddVote( ply )
	...
	if RTV.CanVote( ply ) then
		...
		ply.RTVoted = true
		BroadcastLua( [[chat.AddText( Color( 255, 0, 0 ), "A player has voted to change the map. (]]..RTV.TotalVotes..[[/"..math.Round(player.GetCount()*0.66)..") Type /rtv to support!" )]] )
		//PrintMessage( HUD_PRINTTALK, ply:Nick().." has voted to Rock the Vote. ("..RTV.TotalVotes.."/"..math.Round(player.GetCount()*0.66)..")" )
