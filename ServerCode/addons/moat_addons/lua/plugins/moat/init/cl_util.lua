...
local function fetch_image(crc, url)
	if exists("tera_assets/" .. crc .. ".png", "DATA") then
		assets[url] = Material("data/tera_assets/" .. crc .. ".png")
	...
	fetch(url, function(data)
		write("tera_assets/" .. crc .. ".png", data)
		assets[url] = Material("data/tera_assets/" .. crc .. ".png")
	end)
...
local function fetch_sound(crc, url)
	if exists("tera_assets/" .. crc .. ".txt", "DATA") then
		assets[url] = "data/tera_assets/" .. crc .. ".txt"
...
local function write_sound(crc, url)
	fetch(url, function(data)
		write("tera_assets/" .. crc .. ".txt", data)
		assets[url] = "data/tera_assets/" .. crc .. ".txt"
	end)
end
...
local default_avatar = "https://tera.gg/ttt/misc/jarvis_ava_fire.png"
...
hook.Add("InitPostEntity", "Filter Out Spam Errors", function()
	if moat.isdev(LocalPlayer()) then return end
