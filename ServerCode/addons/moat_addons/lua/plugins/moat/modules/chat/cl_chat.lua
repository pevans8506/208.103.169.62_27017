...
moat_chat.chattype = ""
moat_chat.alpha = 0
moat_chat.header = system.IsOSX() and "TERA TTT | tera.gg"
...
hook("TTTBeginRound", function()
    ...
    local cur_round = (GetConVar("ttt_round_limit"):GetInt() - GetGlobal("ttt_rounds_left")) + 1
...
function moat_chat.InitChat()
    ...
    FRAME.Paint = function(s, w, h)
            ...
            for pl, time in pairs(MOAT_TYPERS) do
                if (IsValid(pl) and pl ~= LocalPlayer()) then
                    ...
                    elseif (PlayersTyping() > 3) then
                        ...
                        chars = {
                            [1] = {str, surface.GetTextSize(str), true}
                        }
                    else
                        str = str .. ", and " .. pl:Nick()
        ...
        if (moat_chat.Theme.CHAT_BG) then return end
        DrawBlur(s, 5)
    ...
    mc.MOVE.Paint = function(s, w, h)
        cdn.DrawImage("https://tera.gg/ttt/misc/chat_resize.png", 0, 0, w, h, Color(255, 255, 255, (50 + s.HoverColor) * mc.alpha))
    end
...
function moat_chat.OpenChat()
    ...
    moat_chat.header = "TERA TTT | " .. GetServerName():sub(1, 18) .. (system.IsOSX() and "" or " | tera.gg")
    local mc = moat_chat
    ...
    if (texttbl.IsItem and texttbl.item_tbl and texttbl.item_tbl.item) then
        ...
        if (itemtbl.item.NameEffect) then
            ...
            elseif (tfx == "threecolors") then
                if (not itemtbl.item.NameEffectMods[4]) then
                    itemtbl.item.NameEffectMods[4] = 1
                end

                if (not itemtbl.item.NameEffectMods[5]) then
                    itemtbl.item.NameEffectMods[5] = RealTime()
                end

                if (itemtbl.item.NameEffectMods[5] <= RealTime()) then
                    itemtbl.item.NameEffectMods[4] = itemtbl.item.NameEffectMods[4] + 1

                    if (itemtbl.item.NameEffectMods[4] > 3) then
                        itemtbl.item.NameEffectMods[4] = 1
                    end
...
function chat.AddText(...)
    ...
    for i = 1, TextTableNum do
        if (not IsValid(LocalPlayer()) or not IsValid(Sender)) then break end
        local t = type(TextTable[i])
    ...
    while pos ~= TextTableNum do
        ...
        if (type(TextTable[pos]) == "table" and TextTable[pos].IsItem and type(TextTable[pos][2]) == "table") then
            ...
            table.insert(FinalText, {
                text, TextPosX, TextPosY, TextTable[pos][2], 1, IgnoreEmoji = TextTable[pos].IgnoreEmoji
            })
            ...
            if TextPosX + x >= windowSizeX then
                ...
                for line in text:gmatch("[^%s]+") do
                    ...
                    if TextPosX + size >= windowSizeX then
                        local data = {
                            t, TextPosX, TextPosY, LastColor, IgnoreEmoji = IgnoreEmoji
                        }
                ...
                local data = {
                    text, TextPosX, TextPosY, LastColor, IgnoreEmoji = IgnoreEmoji
                }
...
hook("PostDrawTranslucentRenderables", function(depth, skybox)
    ...
    for pl, time in pairs(MOAT_TYPERS) do
        if (not IsValid(pl) or pl == LocalPlayer()) then continue end
        if (not pl.PlayerVisible or pl:Team() == TEAM_SPEC or pl:GetNoDraw()) then continue end
