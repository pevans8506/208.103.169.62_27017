...
function Content:HotMount(wid)
	if (steamworks.IsSubscribed(wid)) then
		MsgC(Color(0, 255, 255), "[TERA TTT] ", Color(255, 0, 0), "We're subbed to " .. tostring(wid) .. ".\n")
	...
	steamworks.FileInfo(wid, function(r)
		if (not r) then
			...
			return MsgC(Color(0, 255, 255), "[TERA TTT] ", Color(255, 0, 0), "Failed to Hot Load " .. tostring(wid) .. ".\n")
		end

		steamworks.Download(r.fileid, true, function(c)
			if (not c) then
				if (tries < 10) then self:HotMount(wid) return end
				return MsgC(Color(0, 255, 255), "[TERA TTT] ", Color(255, 0, 0), "Failed to Mount " .. tostring(wid) .. ".\n")
			end

			game.MountGMA(c)

			MsgC(Color(0, 255, 255), "[TERA TTT] ", Color(255, 0, 0), "Hot Loaded " .. tostring(wid) .. ".\n")
		end)
	end)
end
...
function Content:FinishDownloads()
	Content.done = true
	-- MsgC(Color(0, 255, 255), "[TERA TTT] ", Color(0, 255, 0), "Finished Mounting all Addons!\n")
end
