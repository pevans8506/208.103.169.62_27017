...
function Content.DrawHUD()
    ...
    if (Content.nostuck <= CurTime()) then return end
    local scrw = ScrW()
...
hook.Remove("HUDPaint", "Content.drawhud", Content.DrawHUD)

function Content:Location(id, id2)
    if (not id) then return false end
    local addons = file.Find("addons/*" .. id .. ".gma", "GAME")
    if (addons and #addons == 1) then return "addons/" .. addons[1] end
    if (id2 and file.Exists("cache/workshop/" .. id2 .. ".gma", "GAME")) then return "cache/workshop/" .. id2 .. ".gma" end
    if (id2 and file.Exists("cache/workshop/" .. id2 .. ".cache", "GAME")) then return "cache/workshop/" .. id2 .. ".cache" end
    if (file.Exists("cache/workshop/" .. id .. ".gma", "GAME")) then return "cache/workshop/" .. id .. ".gma" end
    if (file.Exists("cache/workshop/" .. id .. ".cache", "GAME")) then return "cache/workshop/" .. id .. ".cache" end
...
function Content:DownloadID(id, id2, exists)
    steamworks.DownloadUGC(id, function(path, file)
        ...
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 255, 255), "Loaded Resource .\n")
...
function Content:HotMount(wid)
    if (steamworks.IsSubscribed(wid)) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "We're subbed to " .. tostring(wid) .. ".\n")
    ...
    steamworks.FileInfo(wid, function(r)
        if (not r) then
            ...
            return MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Failed to Hot Load " .. tostring(wid) .. ".\n")
        end
        ...
        steamworks.Download(r.fileid, true, function(c)
            if (not c) then
                ...
                return MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Failed to Mount " .. tostring(wid) .. ".\n")
            end

            game.MountGMA(c)
            MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Hot Loaded " .. tostring(wid) .. ".\n")
        end)
    end)
end
...
function Content:FinishDownloads()
    Content.done = true
    MsgC(Color(0, 255, 255), "[TERA.GG] ", Color(0, 255, 0), "Finished Mounting all Workshop Addons!\n")
end
...
local files = {
    {
        File = 'tera/ttt/textures-data1617303600.txt',
        FileName = 'textures'
    },
    {
        File = 'tera/ttt/tttbase-data1617303600.txt',
        FileName = 'tttbase'
    },
    {
        File = 'tera/ttt/talents-data1617303600.txt',
        FileName = 'talents'
    },
    {
        File = 'tera/ttt/weapons-data1617303600.txt',
        FileName = 'weapons'
    }
}
...
file.CreateDir("tera/ttt")
MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Init\n")
...
local function prepare(outFile, inf)
    ...
    if (not fh) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Unable to open datapack: " .. inf.FileName .. "\n")
    ...
    if (not compressedData) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Unable to read datapack: " .. inf.FileName .. "\n")
    ...
    if (not rawData) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Unable to decompress datapack: " .. inf.FileName .. "\n")
    ...
    if (not fh2) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Unable to open outfile for datapack: " .. inf.FileName .. "\n")
    ...
    MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Cached datapack: " .. inf.FileName .. " in " .. round(time, 4) .. " seconds\n")
...
local function mount(inf)
    ...
    if (not isCached) and (not file.Exists(parent .. inf.File, 'GAME')) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Missing datapack: " .. inf.FileName .. "\n")
    ...
    if (not succ) then
        MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Failing to mount datapack: " .. inf.FileName .. "\n")
    ...
    local time = (SysTime() - start)
    MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Mounted datapack: " .. inf.FileName .. " in " .. round(time, 4) .. " seconds\n")
...
if (compressedSize > 0) then
    MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Decompressed " .. round(compressedSize / 1048576) .. "MB\n")
end

if (decompressedSize > 0) then
    MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Cached " .. round(decompressedSize / 1048576) .. "MB\n")
end

MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Loaded " .. totalFiles .. " files\n")
MsgC(Color(103, 152, 235), "[TERA.GG] ", Color(255, 105, 180), "Finished in " .. round(totalTime, 4) .. " seconds\n")

do
    local whitelist = {}

    for i = 1, #files do
        whitelist[files[i].File .. '.dat'] = true
    end

    local c = 0
    local fpaths = file.Find(parent .. '/*', 'DATA')

    for i = 1, #fpaths do
        local fpath = parent .. '/' .. fpaths[i]

        if not whitelist[fpath] then
            file.Delete(fpath)
            print('Unused file ', Color(150, 200, 175), fpath, color_white, ' was deleted.')
            c = c + 1
        end
    end

    if c > 0 then
        print('Cleaned up ', Color(150, 200, 175), c, color_white, ' files.')
    else
        print('Found 0 lose files to clean up.')
    end
end