...
function m_OpenCrate(tbl, fastopen, massopen)
    ...
    if not massopen then
        net.Start("MOAT_VERIFY_CRATE")
    else
        net.Start("MOAT_VERIFY_CRATES")
    end
...
function m_InitCrateWindow(itemtbl, item_crate_slot, item_crate_class, preview)
    ...
    if (preview) then
        MOAT_CRATE_FRAME.TitleText = "This is a preview of the crate."
    end
    ...
    MOAT_CRATE.Paint = function(s, w, h)
        ...
        if (itemtbl.item.Image:StartWith("https")) then
            cdn.DrawImage(itemtbl.item.Image, 2, 2, 64, 64, {
                r = 255,
                g = 255,
                b = 255,
                a = 255
    ...
    MOAT_ITEM_PREVIEW.CustomThink = function(s)
        if (not IsValid(MOAT_CRATE_FRAME)) then
            s:Remove()
    ...
    if (custom_preview and IsValid(MOAT_CRATE_PREVIEW)) then
        MOAT_CRATE_PREVIEW:Remove()
    end
...
net.Receive("MOAT_VERIFY_CRATE", function(len)
    ...
    if (IsValid(MOAT_CRATE_BG)) then return end
