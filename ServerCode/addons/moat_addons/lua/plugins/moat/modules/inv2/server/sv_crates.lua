...
net.Receive("MOAT_VERIFY_CRATE", function(len, ply)
    ...
    --if MOAT_CRATES[ply] then return end -- prevent net limiter
    if ply.OpeningCrate then return end
    ply.OpeningCrate = true
    ...
    if (fast_open and da_item and da_item.Price) then
        ...
        ply.OpeningCrate = false

        return
    end

    if (da_item and not da_item.Price) then return end
    ply.OpeningCrate = false
    MOAT_CRATESAVE[ply] = {}
...
local function massopenshit(len, ply)
    if not table.HasValue(MOAT_MVP, ply:GetUserGroup()) then return end
    ply.OpeningCrate = true
    local item_class = net.ReadDouble()
    local fast_open = net.ReadBool()
    local crate_id = 0
...
function PreventNetIssue(len, ply)
    if not IsValid(ply) then return end
    if ply.OpeningCrate then
        timer.Simple(2.5, PreventNetIssue)

        return
    else
        massopenshit(len, ply)
    end
end

util.AddNetworkString("MOAT_VERIFY_CRATES")
net.ReceiveNoLimit("MOAT_VERIFY_CRATES", PreventNetIssue)
