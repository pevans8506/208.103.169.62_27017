...
function plyMeta:canBeMoatFrozen()
    return (IsValid(self) and self:Team() ~= TEAM_SPEC and not GetGlobal("MOAT_MINIGAME_ACTIVE") and GetRoundState() == ROUND_ACTIVE)
end
...
if (SERVER) then
    ...
    net.Receive("Switch_wep_primary", function()
        ...
        if not e.PrintName then
            e.PrintName = ""
        end

        if not n.PrintName then
            n.PrintName = e.PrintName
        end
    ...
    function OMEGA:ViewModelDrawn(old, vm)
        if (self.InsideDraw) then return end
    ...
    function OMEGA:DrawWorldModel(old)
        ...
        if (lp ~= _Owner) then
            if (lp:GetObserverMode() == OBS_MODE_IN_EYE and lp:GetObserverTarget() == _Owner) then return end
        end
    ...
    -- Global
    local mat = CreateMaterial("Moat.Talents.Mat", "VertexLitGeneric", {
    ...
    -- Notify
    local tal_cols = {Color(178, 102, 255, 255), Color(0, 255, 128, 255)}
    ...
    local FriedLocalPlayer, FriedPlayers, FriedScreen = CurTime(), {
        Count = 0,
        Delete = {}
    }, {
        ["$pp_colour_addr"] = 0,
    ...
    hook("RenderScreenspaceEffects", function()
        if (FriedLocalPlayer < CurTime()) then return end
    ...
    hook("PostDrawOpaqueRenderables", function()
        if (FriedPlayers.Count == 0) then return end
    ...
    hook.Add("HUDPaint", "moat_JennyDoggo", function()
        ...
        for i = 1, 13 do
            ...
            if (not doggo_save[i] or (doggo_save[i] and doggo_save[i].lastupdate and doggo_save[i].lastupdate < CurTime() - 0.1)) then
                doggo_save[i] = {
                    math.random(1, scrw - 256), math.random(1, scrh - 256), lastupdate = CurTime()
                }
            end

            cdn.DrawImage("https://web.tera.gg/assets/img/dogs/" .. img .. ".png", doggo_save[i][1], doggo_save[i][2], 256, 256, Color(255, 255, 255))
        end
    end)
