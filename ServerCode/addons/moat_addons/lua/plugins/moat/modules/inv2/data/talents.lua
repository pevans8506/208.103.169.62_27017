...
TALENT.Tier = 1
TALENT.LevelRequired = { min = 7, max = 12 }

TALENT.Modifications = {}
TALENT.Modifications[1] = { min = 8, max = 20 } -- Percent damage is increased by
...
TALENT.Modifications = {}
TALENT.Modifications[1] = { min = 50, max = 70 } -- Percent chance to ignore armour
...
TALENT.Modifications = {}
TALENT.Modifications[1] = {min = 35, max = 75}
TALENT.Melee = false
TALENT.NotUnique = false
...
TALENT.Tier = 1
TALENT.LevelRequired = { min = 8, max = 12 }
...
TALENT.Modifications = {}
TALENT.Modifications[1] = {min = 20, max = 40} -- Damage last bullet can do
...
TALENT.Description = 'After killing a player, you receive a %s_^ damage reduction for %s^ seconds'
TALENT.Tier = 2
TALENT.LevelRequired = {min = 15, max = 20}

TALENT.Modifications = {}
TALENT.Modifications[1] = {min = 10,  max = 20} -- Percent damage reduction
...
function TALENT:OnPlayerDeath(vic, inf, att, talent_mods)
	local percent = (self.Modifications[1].min + ((self.Modifications[1].max - self.Modifications[1].min) * math.min(1, talent_mods[1]))) / 100
...
TALENT.Modifications = {}
TALENT.Modifications[1] = {min = 15, max = 40} -- Damage last bullet can do
...
TALENT.ID = 23
TALENT.Suffix = "the Frenzy"
TALENT.Name = "Frenzy"
TALENT.NameColor = Color(255,255,0)
TALENT.Description = "Speed is increased by %s_^ for 8 seconds and jump height is increased by 20% after every kill"
TALENT.Tier = 2
TALENT.LevelRequired = {min = 15, max = 20}
...
function TALENT:OnPlayerDeath( victim, inf, attacker, talent_mods )
	...
	if attacker:GetJumpPower() <= 550 then
		attacker:SetJumpPower(attacker:GetJumpPower() * 1.2)
	end
	status.Inflict("Speedforce", {Time = 8, Player = attacker, Speed = speed})
end
...
TALENT.ID = 2
TALENT.Name = "Storm trooper"
TALENT.NameColor = Color( 0, 255, 0 )
TALENT.Description = "Damage is increased by %s_^, but "
TALENT.Tier = 2
TALENT.LevelRequired = { min = 15, max = 20 }
...
TALENT.Modifications = {}
TALENT.Modifications[1] = { min = 25, max = 50 }	-- Chance to mute
...
TALENT.ID = 53
TALENT.Name = "Absorber"
TALENT.NameColor = Color(0, 255, 128)
TALENT.Description = "After Killing a player, you have a %s_^ chance to add %s^ health to your total maximum health pool for a total of %s^"
TALENT.Tier = 3
TALENT.LevelRequired = {min = 25, max = 30}

TALENT.Modifications = {}
TALENT.Modifications[1] = {min = 5, max = 15}
TALENT.Modifications[2] = {min = 5, max = 15}
TALENT.Modifications[3] = {min = 15, max = 40}
TALENT.Melee = true
TALENT.NotUnique = true

function TALENT:OnPlayerDeath(vic, inf, att, talent_mods)
	if (GetGlobal("MOAT_MINIGAME_ACTIVE")) or cur_random_round then return end

    local chance = math.Round(self.Modifications[1].min + ((self.Modifications[1].max - self.Modifications[1].min) * math.min(1, talent_mods[1])))
	if (chance > math.random() * 100) then

		att:SetHealth(math.Clamp(att:Health() + amt, 0, 100 + maxamt))
		att:SetMaxHealth(math.Clamp(att:GetMaxHealth() + amt, 0, 100 + maxamt))
	end
end
m_AddTalent(TALENT)
...
TALENT.Name = "egg boy"
TALENT.NameColor = Color( 255, 0, 0 )
TALENT.Description = "You have a %s_^ chance to swap positions with the player you killed"
TALENT.Tier = 1
TALENT.LevelRequired = { min = 1, max = 1 }

TALENT.Modifications = {}
TALENT.Modifications[1] = { min = 30, max = 75 } -- Percent chance to tp
