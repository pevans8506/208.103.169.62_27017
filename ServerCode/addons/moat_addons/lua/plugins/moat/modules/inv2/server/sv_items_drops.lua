...
function meta:m_DropInventoryItem(cmd_item, cmd_class, drop_cosmetics, delay_le_saving, hide_chat, dev_talent_tbl, item_lvl)
    local dropped_item = {}
    ...
    if (not item_name_chosen) then
        for k, v in pairs(drop_table) do
            if (v.Rarity == chosen_rarity and (drop_cosmetics == nil or (((not drop_cosmetics[1] and not COSMETIC_TYPES[v.Kind]) or drop_cosmetics[1]) and ((not drop_cosmetics[2] and (v.ID < 6001 or v.ID > 6500)) or drop_cosmetics[2])))) then
                if (v.Collection == "Retro Collection" or v.Collection == "Summer Event Collection" or v.Collection == "Flower Collection" or v.Collection == "Cinco de Mayo Collection" or v.Collection == "Valentine Collection" or v.Collection == "Holiday Collection" or v.Collection == "Omega Collection" or v.Collection == "Aqua Palm Collection" or v.NotDroppable) then continue end
    ...
    if (#items_to_drop > 0) then
        ...
        if (item_to_drop.Kind == "tier" or item_to_drop.Kind == "Unique") then
            ...
            if (item_to_drop.Collection == "Retro Collection" or item_to_drop.Collection == "Summer Event Collection" or item_to_drop.Collection == "Flower Collection" or item_to_drop.Collection == "Cinco de Mayo Collection" or item_to_drop.Collection == "Valentine Collection" or item_to_drop.Collection == "Pumpkin Collection" or item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Aqua Palm Collection" or item_to_drop.Collection == "Holiday Collection" or item_to_drop.Collection == "New Years Collection") then
            ...
            if (item_to_drop.MinTalents and item_to_drop.MaxTalents and item_to_drop.Talents) then
                ...
                if item_to_drop.ID == 688 then
                    for k, v in ipairs(talents_to_loop) do
                        if k == 3 then
                            talents_chosen[k] = m_GetRandomTalent(3, v, false)
                        else
                            talents_chosen[k] = m_GetRandomTalent(2, v, false)
                        end
                    end
                elseif item_to_drop.ID == 687 then
                    for k, v in ipairs(talents_to_loop) do
                        if k ~= 5 then
                            talents_chosen[k] = m_GetRandomTalent((math.random(1,3)), v, false)
                        else
                            talents_chosen[k] = m_GetRandomTalent(sex, v, false)
                        end
                    end
                else
                    for k, v in ipairs(talents_to_loop) do
        ...
        elseif (item_to_drop.Kind == "Melee") then
            ...
            dropped_item.s.l = item_lvl or 1
            dropped_item.s.x = 0
            dropped_item.t = {}
            local talents_chosen = {}
        ...
        if (dropped_item and dropped_item.u and item_to_drop and item_to_drop.Rarity and not dont_show_chat) then
            if (item_to_drop.Rarity == 6) then
                cdn.PlayURL("https://tera.gg/ttt/drops/ascended.mp3", .5)
                util.GlobalScreenShake(5, 5, 10, 5000)
            elseif (item_to_drop.Rarity == 7) then
                cdn.PlayURL("https://tera.gg/ttt/drops/cosmic.mp3", .5)
                util.GlobalScreenShake(5, 5, 10, 5000)
            elseif (item_to_drop.Rarity == 8) then
                cdn.PlayURL("https://tera.gg/ttt/drops/extinct.mp3", .5)
                util.GlobalScreenShake(5, 5, 10, 5000)
            elseif (item_to_drop.Rarity == 9) then
                cdn.PlayURL("https://tera.gg/ttt/drops/planetary.mp3", .5)
                util.GlobalScreenShake(25, 25, 15, 5000)
            elseif (tonumber(dropped_item.u) == 912 or titan_tier_ids[tostring(dropped_item.u)]) then
                cdn.PlayURL("https://tera.gg/ttt/drops/titan.mp3", .5) -- Titan
            end
        end

        if (not delay_le_saving) then
            delay_le_saving = nil
        end

        self:m_AddInventoryItem(dropped_item, delay_le_saving, dont_show_chat)
    else
        self:ChatPrint("ERROR OBTAINING ITEM! CONTACT THE DEVELOPERS!")
    end
end
...
function m_GetRandomInventoryItem(arg_collection, item)
    ...
    if (item and item.Rarities) then
        ...
        for rarity = 1, 9 do
            if (rarity == 8) then continue end
            local pct = item.Rarities[rarity]
            if (not pct) then continue end
    ...
    if (arg_collection == "50/50 Collection") then
        ...
        if (cached_items[arg_collection] and cached_items[arg_collection][rarity_chosen]) then
            ...
            for k, v in pairs(drop_table) do
                if (v.Rarity == rarity_chosen and v.Kind ~= "Crate" and not COSMETIC_TYPES[v.Kind] and v.Collection ~= "Valentine Collection" and v.Collection ~= "Omega Collection" and v.Collection ~= "Aqua Palm Collection" and v.Collection ~= "Holiday Collection" and v.Collection ~= "Cinco de Mayo Collection" and v.Collection ~= "Flower Collection" and v.Collection ~= "Summer Event Collection" and v.Collection ~= "Retro Collection" and not v.NotDroppable) then
                    table.insert(items_from_collection, v)
                end
            end

            if (not cached_items[arg_collection]) then
                cached_items[arg_collection] = {}
            end
    ...
    if (#items_from_collection > 0) then
        ...
        if (not cached_weapons) then
            cached_weapons = weapons.GetList()
        end

        if (item_to_drop.Kind == "tier" or item_to_drop.Kind == "Unique") then
            dropped_item.w = ""

            if (item_to_drop.Collection == "Retro Collection" or item_to_drop.Collection == "Summer Event Collection" or item_to_drop.Collection == "Flower Collection" or item_to_drop.Collection == "Cinco de Mayo Collection" or item_to_drop.Collection == "Valentine Collection" or item_to_drop.Collection == "Pumpkin Collection" or item_to_drop.Collection == "Holiday Collection" or item_to_drop.Collection == "Omega Collection" or item_to_drop.Collection == "Aqua Palm Collection") then
...
hook.Add("DoPlayerDeath", "moat_DropsEndRound", function(ply, att, dmg)
    ...
    if (GetRoundState() == ROUND_ACTIVE and IsRightful(att, ply)) then
...
function m_ResetPaintItem(pl, wep_slot, itemtbl)
    local ply_item = MOAT_INVS[pl]["slot" .. wep_slot]
    local paintid

    if ply_item.p3 and ItemIsSkin(ply_item.p3) then
        paintid = tonumber(ply_item.p3)
        ply_item.p3 = nil
        pl:m_DropInventoryItem(MOAT_PAINT.Skins[paintid][1], "hide_chat_obtained")
    elseif ply_item.p2 and ItemIsPaint(ply_item.p2) then
        paintid = tonumber(ply_item.p2)
        ply_item.p2 = nil
        pl:m_DropInventoryItem(MOAT_PAINT.Paints[paintid][1], "hide_chat_obtained")
    elseif ply_item.p and ItemIsTint(ply_item.p) then
        paintid = tonumber(ply_item.p)
        ply_item.p = nil
        pl:m_DropInventoryItem(MOAT_PAINT.Tints[paintid][1], "hide_chat_obtained")
    end

    if paintid == nil then return end
    m_SaveInventory(pl)
    m_SendInvItem(pl, wep_slot)
end
...
function m_ResetTalents(pl, wep_slot, itemtbl)
    ...
    if ((itemtbl.MinTalents and itemtbl.MaxTalents and itemtbl.Talents) or (itemtbl.Kind and itemtbl.Kind == "Melee")) then
        ...
        elseif itemtbl.ID == 687 then
            for k, v in ipairs(talents_to_loop) do
                if k >= 3 then
                    talents_chosen[k] = m_GetRandomTalent(3, v, false)
                else
                    talents_chosen[k] = m_GetRandomTalent(1, v, false)
                end
            end
        else
            for k, v in ipairs(talents_to_loop) do
...
function m_OldMeleeReset(pl, slot, class)
    ...
    if (not ply_inv["slot" .. slot] or not ply_inv["slot" .. slot].c or not ply_inv["slot" .. slot].s) then return false end
    if (tonumber(ply_inv["slot" .. slot].c) ~= class) then return false end

    if (ply_inv["slot" .. slot].u == 17699) then
        if (ply_inv["slot" .. slot].s.p and ply_inv["slot" .. slot].s.w) then return false end
    ...
    m_FinishUsableItem(pl, {
        Name = "your free stat re-roll! Your melee now has Weight!"
    })
...
net.Receive("OldMelee.Reset", function(l, pl)
    ...
    if (pl.MeleeRolling) then return end
