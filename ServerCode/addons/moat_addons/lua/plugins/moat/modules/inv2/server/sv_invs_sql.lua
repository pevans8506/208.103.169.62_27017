...
function m_InsertCompTicket(c, cb, cbf)
    local q = MINVENTORY_MYSQL:query("INSERT INTO moat_comps ( time, steamid, admin, link, item, class, talent1, talent2, talent3, talent4, comment, approved ) VALUES ( UNIX_TIMESTAMP(), '" .. MINVENTORY_MYSQL:escape(util.SafeSteamID(c.steamid)) .. "', '" .. MINVENTORY_MYSQL:escape(c.admin) .. "', '" .. MINVENTORY_MYSQL:escape(c.ticket) .. "', '" .. MINVENTORY_MYSQL:escape(c.item) .. "', '" .. MINVENTORY_MYSQL:escape(c.class) .. "', '" .. MINVENTORY_MYSQL:escape(c.talent1) .. "', '" .. MINVENTORY_MYSQL:escape(c.talent2) .. "', '" .. MINVENTORY_MYSQL:escape(c.talent3) .. "', '" .. MINVENTORY_MYSQL:escape(c.talent4) .. "', '" .. MINVENTORY_MYSQL:escape(c.comments) .. "', '0')")
...
function TRADES.Print(trade)
    ...
    for num = 1, 10 do
        ...
        if (i.c) then end -- print(TRADES.ItemToFormat(i, "    ", "  "))
    end
end
...
function m_SendInventoryToPlayer(ply)
    ...
    if (ply.Sending) then return end
...
function m_InsertNewInventoryPlayer(ply)
    ...
    local weps = {"weapon_zm_improvised", "weapon_ttt_mp5", "weapon_zm_shotgun", "weapon_ttt_p90", "weapon_ttt_ak47", "weapon_ttt_m16", "weapon_ttt_shotgun", "weapon_zm_rifle", "weapon_ttt_glock", "weapon_ttt_sg552", "weapon_ttt_tmp", "weapon_ttt_galil", "weapon_ttt_famas", "weapon_ttt_aug", "weapon_ttt_dual_elites", "weapon_zm_revolver", "weapon_zm_pistol", "weapon_zm_mac10", "weapon_zm_sledge"}
    ...
    for k, v in pairs(mi.Config.DefaultInv) do
        ply:m_DropInventoryItem(v, nil, nil, nil, true)
    end
end
    ...
    function query1:onSuccess(data)
        if (#data > 0) then
            ...
            for k, v in ipairs(loadout) do
                if (v.u and tonumber(math.Round(v.u)) == 25) then
                    v.u = 600
                end
            end
            ...
            for k, v in ipairs(loadout) do
                if (v.w and not weapons.Get(v.w)) then
                    if (v.w == "weapon_ttt_ak44") then
                        v.w = "weapon_ttt_ak47"
                        continue
                    end

                    if (v.w == "weapon_ttt_te_ak44") then
                        v.w = "weapon_ttt_te_ak47"
                        continue
                    end

                    if (v.w == "weapon_ttt_cz45") then
                        v.w = "weapon_ttt_cz75"
                        continue
                    end
            ...
            local function update_loadout_talent(int, Talents)
                ...
                for i = 1, #loadout[int].t do
                    ...
                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 20202) then
                        loadout[int].t[i].e = 10102
                        continue
                    end

                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 20203) then
                        loadout[int].t[i].e = 10103
                        continue
                    end

                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 9972) then
                        loadout[int].t[i].e = 9971
                        continue
                    end

                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 202) then
                        loadout[int].t[i].e = 102
                        continue
                    end

                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 255) then
                        loadout[int].t[i].e = 155
                        continue
                    end

                    if (loadout[int].t[i] and loadout[int].t[i].e and loadout[int].t[i].e == 84) then
                        loadout[int].t[i].e = 87
                        continue
                    end
                    ...
                    if ((not Talents[i] or (Talents[i] and not next(Talents[i]))) and MOAT_DROPTABLE[loadout[int].u].Talents[i]) then
                        ...
                        loadout[int].t[i] = {
                            e = Talents[i].ID,
                            l = loadout[int].t[i].l or math.random(Talents[i].LevelRequired and Talents[i].LevelRequired.min or (Talents[i].Tier * 10), Talents[i].LevelRequired and Talents[i].LevelRequired.max or (Talents[i].Tier * 20))
                        }
                    ...
                    elseif (Talents[i] and Talents[i].Modifications and loadout[int].t[i] and (not loadout[int].t[i].m or loadout[int].t[i].m) and (((not loadout[int].t[i].m) or (loadout[int].t[i].m and #Talents[i].Modifications ~= #loadout[int].t[i].m)) or i ~= Talents[i].Tier or (MOAT_DROPTABLE[loadout[int].u].Talents and MOAT_DROPTABLE[loadout[int].u].Talents[i] ~= "random")) and (not MOAT_DROPTABLE[loadout[int].u].Talents or ((MOAT_DROPTABLE[loadout[int].u].Talents and MOAT_DROPTABLE[loadout[int].u].Talents[i] ~= Talents[i].Name) or ((not loadout[int].t[i].m) or (loadout[int].t[i].m and #Talents[i].Modifications ~= #loadout[int].t[i].m))))) then
                        ...
                        if (loadout[int].t[i].e == 2 and ((loadout[int].t[i].m and #loadout[int].t[i].m == 2) or i == 1)) then
                            loadout[int].t[i].e = 1
                        end

                        if (loadout[int].t[i].e == 1 and (loadout[int].t[i].m and #loadout[int].t[i].m == 1)) then
                            loadout[int].t[i].e = 2
                        end

                        if (loadout[int].t[i].e == 4 and i == 2) then
                            loadout[int].t[i].e = 7
                        end

                        if (loadout[int].t[i].e == 7 and i == 1) then
                            loadout[int].t[i].e = 4
                        end

                        if (loadout[int].t[i].e == 26 and (loadout[int].t[i].m and #loadout[int].t[i].m == 2)) then
                            loadout[int].t[i].e = 16
                        end

                        if (loadout[int].t[i].e == 16 and (loadout[int].t[i].m and #loadout[int].t[i].m == 3)) then
                            loadout[int].t[i].e = 26
                        end

                        if (loadout[int].t[i].e == 34 and (loadout[int].t[i].m and #loadout[int].t[i].m == 2)) then
                            loadout[int].t[i].e = 37
                        end

                        if (loadout[int].t[i].e == 23 and (loadout[int].t[i].m and #loadout[int].t[i].m == 1)) then
                            loadout[int].t[i].e = 13
                        end

                        if (loadout[int].t[i].e == 13 and (loadout[int].t[i].m and #loadout[int].t[i].m == 2)) then
                            loadout[int].t[i].e = 23
                        end

                        if (loadout[int].t[i].e == 82 and (loadout[int].t[i].m and #loadout[int].t[i].m == 3)) then
                            loadout[int].t[i].e = 81
                        end

                        if (loadout[int].t[i].e == 84) then
                            loadout[int].t[i].e = 87
                        end

                        if (loadout[int].t[i].e == 32 and i == 1) then
                            loadout[int].t[i].e = 31
                        end

                        if (loadout[int].t[i].e == 29 and i == 2) then
                            loadout[int].t[i].e = 19
                        end

                        if (loadout[int].t[i].e == 24 and i == 2) then
                            loadout[int].t[i].e = 14
                        end

                        if (loadout[int].t[i].e == 24 and i == 3) then
                            loadout[int].t[i].e = 17
                        end

                        if (loadout[int].t[i].e == 21 and i == 2) then
                            loadout[int].t[i].e = 22
                        end

                        if (loadout[int].t[i].e == 22 and i == 1) then
                            if (#loadout[int].t[i].m == 2) then
                                loadout[int].t[i].e = 21
                            else
                                loadout[int].t[i].e = 12
                            end
                        end

                        if ((loadout[int].t[i].e == 27 or loadout[int].t[i].e == 24 or loadout[int].t[i].e == 14) and i == 3) then
                            loadout[int].t[i].e = 17
                        end
            ...
            for i = 1, #loadout do
                ...
                if loadout[i].u == 688 or loadout[i].u == 687 then continue end
                ...
                if (loadout[i].u and MOAT_DROPTABLE[loadout[i].u] and MOAT_DROPTABLE[loadout[i].u].Talents and (not loadout[i].t or (type(loadout[i].t) == "table" and #MOAT_DROPTABLE[loadout[i].u].Talents > #loadout[i].t))) then
                    ...
                    for k, v in ipairs(MOAT_DROPTABLE[loadout[i].u].Talents) do
                        ...
                        if (not loadout[i].t[k].e) then
                            ...
                            loadout[i].t[k] = {
                                e = new.ID,
                                l = loadout[i].t[k].l or math.random(new.LevelRequired and new.LevelRequired.min or (new.Tier * 10), new.LevelRequired and new.LevelRequired.max or (new.Tier * 20))
                            }
            ...
            for i = 1, #inventory_tbl do
                if (inventory_tbl[i].w and not weapons.Get(inventory_tbl[i].w)) then
                    if (inventory_tbl[i].w == "weapon_ttt_ak44") then
                        inventory_tbl[i].w = "weapon_ttt_ak47"
                        continue
                    end

                    if (inventory_tbl[i].w == "weapon_ttt_te_ak44") then
                        inventory_tbl[i].w = "weapon_ttt_te_ak47"
                        continue
                    end

                    if (inventory_tbl[i].w == "weapon_ttt_cz45") then
                        inventory_tbl[i].w = "weapon_ttt_cz75"
                        continue
                    end
            ...
            local function update_talent(int, Talents)
                ...
                for i = 1, #inventory_tbl[int].t do
                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 20202) then
                        inventory_tbl[int].t[i].e = 10102
                        continue
                    end

                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 20203) then
                        inventory_tbl[int].t[i].e = 10103
                        continue
                    end

                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 9972) then
                        inventory_tbl[int].t[i].e = 9971
                        continue
                    end

                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 202) then
                        inventory_tbl[int].t[i].e = 102
                        continue
                    end

                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 255) then
                        inventory_tbl[int].t[i].e = 155
                        continue
                    end

                    if (inventory_tbl[int].t[i] and inventory_tbl[int].t[i].e and inventory_tbl[int].t[i].e == 84) then
                        inventory_tbl[int].t[i].e = 87
                        continue
                    end
                    ...
                    if ((not Talents[i] or (Talents[i] and not next(Talents[i]))) and (MOAT_DROPTABLE[inventory_tbl[int].u].Talents and MOAT_DROPTABLE[inventory_tbl[int].u].Talents[i])) then
                        ...
                        inventory_tbl[int].t[i] = {
                            e = Talents[i].ID,
                            l = inventory_tbl[int].t[i].l or math.random(Talents[i].LevelRequired and Talents[i].LevelRequired.min or (Talents[i].Tier * 10), Talents[i].LevelRequired and Talents[i].LevelRequired.max or (Talents[i].Tier * 20))
                        }
                    ...
                    elseif (Talents[i] and Talents[i].Modifications and inventory_tbl[int].t[i] and (not inventory_tbl[int].t[i].m or inventory_tbl[int].t[i].m) and (((not inventory_tbl[int].t[i].m) or (inventory_tbl[int].t[i].m and #Talents[i].Modifications ~= #inventory_tbl[int].t[i].m)) or i ~= Talents[i].Tier or (MOAT_DROPTABLE[inventory_tbl[int].u].Talents and MOAT_DROPTABLE[inventory_tbl[int].u].Talents[i] ~= "random")) and (not MOAT_DROPTABLE[inventory_tbl[int].u].Talents or (MOAT_DROPTABLE[inventory_tbl[int].u].Talents and (MOAT_DROPTABLE[inventory_tbl[int].u].Talents[i] ~= Talents[i].Name or ((not inventory_tbl[int].t[i].m) or (inventory_tbl[int].t[i].m and #Talents[i].Modifications ~= #inventory_tbl[int].t[i].m)))))) then
                        -- print("yo", i, int, #Talents[i].Modifications ~= #inventory_tbl[int].t[i].m, i ~= Talents[i].Tier, MOAT_DROPTABLE[inventory_tbl[int].u].Talents[i] ~= "random", MOAT_DROPTABLE[inventory_tbl[int].u].Talents[i] ~= Talents[i].Name)
                        if (inventory_tbl[int].t[i].e == 2 and ((inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 2) or i == 1)) then
                            inventory_tbl[int].t[i].e = 1
                        end

                        if (inventory_tbl[int].t[i].e == 1 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 1)) then
                            inventory_tbl[int].t[i].e = 2
                        end

                        if (inventory_tbl[int].t[i].e == 4 and i == 2) then
                            inventory_tbl[int].t[i].e = 7
                        end

                        if (inventory_tbl[int].t[i].e == 7 and i == 1) then
                            inventory_tbl[int].t[i].e = 4
                        end

                        if (inventory_tbl[int].t[i].e == 26 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 2)) then
                            inventory_tbl[int].t[i].e = 16
                        end

                        if (inventory_tbl[int].t[i].e == 16 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 3)) then
                            inventory_tbl[int].t[i].e = 26
                        end

                        if (inventory_tbl[int].t[i].e == 34 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 2)) then
                            inventory_tbl[int].t[i].e = 37
                        end

                        if (inventory_tbl[int].t[i].e == 23 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 1)) then
                            inventory_tbl[int].t[i].e = 13
                        end

                        if (inventory_tbl[int].t[i].e == 13 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 2)) then
                            inventory_tbl[int].t[i].e = 23
                        end

                        if (inventory_tbl[int].t[i].e == 82 and (inventory_tbl[int].t[i].m and #inventory_tbl[int].t[i].m == 3)) then
                            inventory_tbl[int].t[i].e = 81
                        end

                        if (inventory_tbl[int].t[i].e == 84) then
                            inventory_tbl[int].t[i].e = 87
                        end

                        if (inventory_tbl[int].t[i].e == 32 and i == 1) then
                            inventory_tbl[int].t[i].e = 31
                        end

                        if (inventory_tbl[int].t[i].e == 29 and i == 2) then
                            inventory_tbl[int].t[i].e = 19
                        end

                        if (inventory_tbl[int].t[i].e == 24 and i == 2) then
                            inventory_tbl[int].t[i].e = 14
                        end

                        if (inventory_tbl[int].t[i].e == 24 and i == 3) then
                            inventory_tbl[int].t[i].e = 17
                        end

                        if (inventory_tbl[int].t[i].e == 21 and i == 2) then
                            inventory_tbl[int].t[i].e = 22
                        end

                        if (inventory_tbl[int].t[i].e == 22 and i == 1) then
                            if (#inventory_tbl[int].t[i].m == 2) then
                                inventory_tbl[int].t[i].e = 21
                            else
                                inventory_tbl[int].t[i].e = 12
                            end
                        end

                        if ((inventory_tbl[int].t[i].e == 27 or inventory_tbl[int].t[i].e == 24 or inventory_tbl[int].t[i].e == 14) and i == 3) then
                            inventory_tbl[int].t[i].e = 17
                        end
            ...
            for i = 1, #inventory_tbl do
                ...
                if inventory_tbl[i].u == 688 or inventory_tbl[i].u == 687 then continue end
                ...
                if (inventory_tbl[i].u and MOAT_DROPTABLE[inventory_tbl[i].u] and MOAT_DROPTABLE[inventory_tbl[i].u].Talents and (not inventory_tbl[i].t or (type(inventory_tbl[i].t) == "table" and #MOAT_DROPTABLE[inventory_tbl[i].u].Talents > #inventory_tbl[i].t))) then
                    ...
                    for k, v in ipairs(MOAT_DROPTABLE[inventory_tbl[i].u].Talents) do
                        ...
                        if (not inventory_tbl[i].t[k].e) then
                            ...
                            inventory_tbl[i].t[k] = {
                                e = new.ID,
                                l = inventory_tbl[i].t[k].l or math.random(new.LevelRequired and new.LevelRequired.min or (new.Tier * 10), new.LevelRequired and new.LevelRequired.max or (new.Tier * 20))
                            }
