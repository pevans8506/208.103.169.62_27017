...
function m_AddDroppableItem(item_table, item_kind)
    ...
    if (SERVER and item_table.Collection) then
        if (not MOAT_COLLECT[item_table.Collection]) then
            MOAT_COLLECT[item_table.Collection] = {}
        end
            ...
            for k, v in pairs(tbl.Stats) do
                if ((tbl.Kind == "tier" or tbl.Kind == "Tier" or tbl.Kind == "Unique") and mi.Stat.Roster[k]) then
                    if (mi.Stat.Roster[k].Defaults[def] and mi.Stat.Roster[k].Defaults[def][tbl.Rarity]) then
                        if (mi.Stat.Roster[k].Defaults[def][tbl.Rarity].Min == v.min and mi.Stat.Roster[k].Defaults[def][tbl.Rarity].Max == v.max) then continue end
                    end
                end
        ...
        if (tbl.ModifyClientsideModel and file_str) then
            ...
            for i = 1, #tb do
                ...
                if (string.len(tb[i]) <= 1) then continue end
                fs = fs .. "		" .. tb[i] .. "\n"
            end
        ...
        elseif (tbl.ItemUsed and file_str) then
            ...
            if (not sf) then
                sf = string.find(file_str, "ITEM.ItemUsed")
            end
        ...
        if (tbl.Price) then
            ...
            table.insert(MOAT_COLLECT[item_table.Collection], {
                String = str,
                Rarity = tbl.Rarity
            })
        end
...
function m_CreatePaints()
    for k, v in pairs(MOAT_PAINT.Tints) do
        ...
        tbl.PaintVer = 1
        tbl.Image = v[4] or "https://tera.gg/ttt/misc/tint.png"
        tbl.ItemCheck = 11
    ...
    for k, v in pairs(MOAT_PAINT.Paints) do
        ...
        tbl.Collection = "Paint Collection"
        tbl.PaintVer = 2
        tbl.Image = v[4] or "https://tera.gg/ttt/misc/paint.png"
        tbl.ItemCheck = 10
    ...
    for k, v in pairs(MOAT_PAINT.Skins) do
        ...
        tbl.PaintVer = 2
        tbl.Image = v[4] or "https://cdn.notfound.tech/2198b5d9d5c8a1e35fe2a4c833556fd6.png"
        tbl.ItemCheck = 12
...
function GetCrateContents(crate_collection)
    if (crate_cache[crate_collection]) then return crate_cache[crate_collection] end
    local contents = {}
...
function GetItemFromEnum(ienum)
    if (ienum and item_cache[ienum]) then return item_cache[ienum] end
...
function GetItemFromEnumWithFunctions(ienum)
    if (ienum and item_cache2[ienum]) then return item_cache2[ienum] end
