...
function jackpot_()
    ...
    function gglobalchat_planetary(name,gun)
        ...
        gun = GetItemFromEnum(gun.u)

        local s = name .. "{420}" .. tostring(gun.Name)
...
local function chat_()
    ...
    function gglobalchat_real(msg)
		...
		discord.Send("TERA TTT Announcement", markdown.WrapBold(
                ...
                if tostring(v.steamid) == "-1337" then
                    ...
                    elseif v.msg == "[EndRound]" then
						...
						elseif (GetServerName():lower():match("minecraft")) then
							...
							local msg = (GetHostName() or "") .. " ( steam://connect/" .. (game.GetIP() or "") .. " ) is switching map to `" .. 'ttt_innocentmotel_v1' .. "`"
							discord.Send("Server", msg)

							RunConsoleCommand("changelevel", "ttt_innocentmotel_v1")
						end
                    else
                        net.Start("Moat.GlobalAnnouncement")
