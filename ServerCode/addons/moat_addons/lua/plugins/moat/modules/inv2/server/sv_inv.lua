    ...
    for k, v in SortedPairs(itemtbl.s) do
        ...
        if (not table.HasValue(default_stats, stat_str) and not table.HasValue(level_stats, stat_str)) then
            table.insert(embed.fields,{
                ...
                value = ("[▰](https://tera.gg)"):rep(plus) .. ("▱"):rep(20 - plus)  ,
                inline = false
            })
        end
    end
    if (itemtbl.t) then
        ...
        for k, v in ipairs(itemtbl.t) do
            ...
            for i = 1, table.Count(v.m) do
                local mod_num = "[" .. math.Round(itemtbl.Talents[k].Modifications[i].min + ((itemtbl.Talents[k].Modifications[i].max - itemtbl.Talents[k].Modifications[i].min) * math.min(1, v.m[i])), 1) .. "](https://tera.gg)"
...
local function discord_drop(ply,item,gift)
    http.Fetch("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=1A1B81A379F749EFFA22927033E0ED34&steamids=" .. ply:SteamID64() .. "&format=json", function(body, size, headers, code)
...
function m_InitTradeAccept(trade_id)
    ...
    if (not offer_player1:m_HasIC(offer_table1_ic)) then 
        ...
        discord.Send("Trade", "Hey, " .. offer_player1:SteamID() .. " tried to trade IC they don't have with " .. offer_player2:SteamID() .. "! <@200418142399234048>")
    ...
    if (not offer_player2:m_HasIC(offer_table2_ic)) then
        ...
        discord.Send("Trade", "Hey, " .. offer_player2:SteamID() .. " tried to trade IC they don't have with " .. offer_player1:SteamID() .. "! <@200418142399234048>")
        MOAT_TRADES[trade_id] = nil
        return
    end
    ...
    if (MOAT_TRADE_BANNED and (MOAT_TRADE_BANNED[offer_player1:SteamID()] or MOAT_TRADE_BANNED[offer_player2:SteamID()])) then
        if (MOAT_TRADE_BANNED[offer_player1:SteamID()]) then
            discord.Send("Trade", "Hey, " .. offer_player1:SteamID() .. " traded with " .. offer_player2:SteamID() .. " when they're not supposed to! <@200418142399234048>")
        else
            discord.Send("Trade", "Hey, " .. offer_player2:SteamID() .. " traded with " .. offer_player1:SteamID() .. " when they're not supposed to! <@200418142399234048>")
        end
    end
...
net.Receive("MOAT_TRADE_CREDITS", function(len, ply)
    ...
    if (credits ~= credits) or (trade_id ~= trade_id) then
        local msg = ply:Nick() .. " (" .. ply:SteamID() .. ") Tried to add nan IC to trade. <@150809682318065664> <@139152491496472576>"
...
hook.Add("PlayerDeconstructedItem", "moat_deconstruct_items", function(pl, enum)
	...
	if (string.find(string.lower(pl:Nick()), "tera.gg")) then
    ...
    if (table.HasValue(MOAT_MVP, pl:GetUserGroup())) then
		multiplier = multiplier + .25
    end
