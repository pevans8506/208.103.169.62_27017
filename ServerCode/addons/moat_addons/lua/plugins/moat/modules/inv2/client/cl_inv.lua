MOAT_VERSION = "Tera v1"
MOAT_BG_URL = "https://tera.gg/ttt/misc/bg.png"
...
function m_isUsingInv(inv)
    if (inv) then return IsValid(MOAT_INV_BG) and IsValid(M_INV_PNL) end
...
local function DrawSnow(pnl, w, h, amt)
    ...
    for i = 1, amt do
        ...
        cdn.SmoothImage("https://tera.gg/ttt/misc/snow.png", snowtbl[i][2], snowtbl[i][1] - (snowtbl[i][3] / 2), snowtbl[i][3], snowtbl[i][3], Color(230, 230, 250, 200))
    end

    cdn.SmoothImage("https://tera.gg/ttt/misc/snow2.png", 0, h - 256, 256, 256, Color(230, 230, 250, 255))
end
...
local spooks = {
    [1] = {
        "1left.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/1left.png"
    },
    [2] = {
        "2left.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/2left.png"
    },
    [3] = {
        "3left.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/3left.png"
    },
    [4] = {
        "1right.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/1right.png"
    },
    [5] = {
        "2right.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/2right.png"
    },
    [6] = {
        "3right.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/3right.png"
    }
}
...
local spring_bg_url = "https://tera.gg/ttt/misc/spring_bg.png"

local springs = {
    [1] = {
        "butterfly1.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/butterfly1.png"
    },
    [2] = {
        "butterfly2.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/butterfly2.png"
    },
    [3] = {
        "butterfly3.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/butterfly3.png"
    },
    [4] = {
        "butterfly4.png", 0, 0, 0, url = "https://tera.gg/ttt/misc/butterfly4.png"
    }
}
    ...
    for k, v in ipairs(desc_cache[text]) do
        if (type(v) == "string") then
            ...
            desc_cache[text][k] = {
                Text = str,
                Width = strw,
                Color = string.match(str, "^[%.%-%+]?[%d*%%*]%.?[^%)]*[%s*%d*][%.%-%+%%]?$") and number_col or col
            }
...
function m_DrawItemStats(font, x, y, itemtbl, pnl)
    ...
    if (not pnl.savewide) then
        pnl.savewide = pnl:GetWide()
    end
    ...
    for k, v in SortedPairs(itemtbl.s) do
        if (stats_drawn[tostring(k)]) then continue end
    ...
    if (itemtbl.t) then
        ...
        for k, v in ipairs(itemtbl.t) do
            ...
            elseif (tfx == "threecolors") then
                if (not itemtbl.Talents[k].NameEffectMods[4]) then
                    itemtbl.Talents[k].NameEffectMods[4] = 1
                end

                if (not itemtbl.Talents[k].NameEffectMods[5]) then
                    itemtbl.Talents[k].NameEffectMods[5] = RealTime()
                end
                ...
                if (itemtbl.Talents[k].NameEffectMods[5] <= RealTime()) then
                    ...
                    if (itemtbl.Talents[k].NameEffectMods[4] > 3) then
                        itemtbl.Talents[k].NameEffectMods[4] = 1
                    end
            ...
            if (itemtbl.s.l < talent_level) then
                ...
                cdn.DrawImage("https://tera.gg/ttt/misc/icon-lock.png", 4 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 3, 10, 10, Color(0, 0, 0, 255))
                cdn.DrawImage("https://tera.gg/ttt/misc/icon-lock.png", 3 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 2, 10, 10, Color(100, 100, 100, 255))
                talent_namew = talent_namew + 8 + 4
            else
                local lx, ty = 4 + draw_name_x + talent_namew, y + stats_y_add + talents_y_add + 2 + 2
                cdn.DrawImage("https://tera.gg/ttt/misc/icon_star.png", lx, ty, 12, 12, Color(0, 0, 0, 255))
                cdn.DrawImage("https://tera.gg/ttt/misc/icon_star.png", lx - 1, ty - 1, 12, 12, Color(255, 255, 255, 255))
            ...
            if (#v.m > 0) then
                for i = 1, #v.m do
                    if (not itemtbl.Talents[k].Modifications or not itemtbl.Talents[k].Modifications[i]) then continue end
...
MOAT_ITEMS_CRATE_MARKED = MOAT_ITEMS_CRATE_MARKED or 0
...
if (NUMBER_OF_SLOTS == 0 or #m_Inventory < NUMBER_OF_SLOTS) then
    net.Start("MOAT_SEND_INV_ITEM")
    net.SendToServer()
...
rarity_names = {
    [0] = {
        "Stock", Color():SetHex("#606e88"), {
            min = 10,
            max = 20
        }
    },
    [1] = {
        "Worn", Color():SetHex("#ccccff"), {
            min = 10,
            max = 20
        }
    },
    [2] = {
        "Standard", Color():SetHex("#3976f4"), {
            min = 20,
            max = 40
        }
    },
    [3] = {
        "Specialized", Color():SetHex("#ba33ff"), {
            min = 60,
            max = 120
        }
    },
    [4] = {
        "Superior", Color():SetHex("#ff00e7"), {
            min = 240,
            max = 480
        }
    },
    [5] = {
        "High-End", Color():SetHex("#fd0b30"), {
            min = 1200,
            max = 2400
        }
    },
    [6] = {
        "Ascended", Color():SetHex("#ffe300"), {
            min = 7200,
            max = 14400
        }
    },
    [7] = {
        "Cosmic", Color():SetHex("#01ff1f"), {
            min = 25200,
            max = 50400
        }
    },
    [8] = {
        "Extinct", Color():SetHex("#ff8a00"), {
            min = 2,
            max = 5000
        }
    },
    [9] = {
        "Planetary", Color(0, 0, 0, 255), {
            min = 25200,
            max = 50400
        }
    }
...
hook.Add("Think", "moat_InventoryHSV", function()
    local hsl = {HSVToColor(CurTime() * 70 % 360, 1, 1), HSVToColor(CurTime() * 70 % 360, .8, 1), HSVToColor(CurTime() * 70 % 360, 1, .8), HSVToColor(CurTime() * 70 % 360, 1, .3), HSVToColor(CurTime() * 70 % 360, 1, .2)}
...
function m_OpenInventory(ply2, utrade)
    ...
    MOAT_ITEMS_CRATE_MARKED = 0
    MOAT_CRATE_ITEMS_START = 0
    MOAT_CRATE_ITEMS_END = 0
    m_HoveredSlot = 0
    HoveringSlot = false
    ...
    MOAT_INV_BG = ux.Create("DFrame", function(s)
        ...
        Paint = function(s, w, h)
            if (MT[CurTheme].BG_PAINT) then return MT[CurTheme].BG_PAINT(s, w, h) end
        ...
        DisableClipping(false)]]
            ...
            cdn.DrawImage("https://tera.gg/ttt/inv_bg.png", 0, 0, 1024, 1024, Color(255, 255, 255, 250))
        end,
        Think = function(s)
            ...
            if (not IsValid(M_SLOT_D)) then return end
            ...
            if (M_INV_DRAG) then
                ...
                if (not M_SLOT_D:IsVisible()) then
    ...
    function m_TradingPanel()
        ...
        function m_AddPlayerTradeList(v)
            for k, pnl in ipairs(M_TRADE_PLYTBL) do
                if (IsValid(pnl) and pnl.Ply == v) then return end
            end
        ...
        function M_TRADE_PLYS:RebuildList()
            ...
            if (M_TRADE_PLYS.Waiting) then return end
    ...
    function m_ShopPanel()
        ...
        M_SHOP_PNL.Paint = function(s, w, h) end --local box_x = 5 --local box_y = 30 --draw_RoundedBox(0, 5, 30, w - 10, h - 35, Color(0, 0, 0, 150))
    ...
    function m_LoadoutPanel(init)
        ...
        function m_CreateLoadoutSlot(num)
            ...
            if (m_ItemExists and m_WClass) then
                if (not string.EndsWith(m_WClass.WorldModel, ".mdl")) then
                    if (not IsValid(m_DPanelIcon.SIcon.Icon)) then
                        m_DPanelIcon.SIcon:CreateIcon(true)
                    end
                ...
                if (m_Loadout[num].c) then
                    if (not string.EndsWith(m_DPanelIcon.WModel, ".mdl")) then
                        ...
                        if (m_DPanelIcon.Item and m_DPanelIcon.Item.item and m_DPanelIcon.Item.item.Clr) then
                            cdn.DrawImage(m_DPanelIcon.WModel, 1, 1, w, h, {
                                r = m_DPanelIcon.Item.item.Clr[1],
                                g = m_DPanelIcon.Item.item.Clr[2],
                                b = m_DPanelIcon.Item.item.Clr[3],
                                a = 255
                            })
                    ...
                    if (m_Loadout[num].marked) then
                        surface_SetDrawColor(132, 222, 2, 200)
                        surface_DrawRect(0, 0, w, h)
                    end
            ...
            m_DPanelBTN.OnMousePressed = function(s, key)
                if (m_DPanelIcon and m_DPanelIcon.SIcon and key == MOUSE_LEFT) then
                    m_DPanelIcon.SIcon:DoClick()
                end

                if (key == MOUSE_LEFT) and (m_DPanelIcon.Item ~= nil) then
                    M_INV_DRAG = M_LOAD_SLOT[num]
                    sfx.Click1()
                    --m_DPanelIcon.SIcon:SetVisible( false )
                    local itemtbl = m_Inventory[num]

                    if (GetConVar("tera_doubleclickopencrate") and GetConVar("tera_doubleclickopencrate"):GetInt() == 1) then
                        if (crate_wait < CurTime()) then
                            if (itemtbl.l ~= 1) then
                                if not (MOAT_ITEMS_CRATE_MARKED) then
                                    MOAT_ITEMS_CRATE_MARKED = 0
                                end

                                MOAT_ITEMS_CRATE_MARKED = MOAT_ITEMS_CRATE_MARKED + 1

                                timer.Simple(0.2, function()
                                    MOAT_ITEMS_CRATE_MARKED = 0
                                end)

                                if (((MOAT_ITEMS_CRATE_MARKED) and (MOAT_ITEMS_CRATE_MARKED >= 2 and MOAT_ITEMS_CRATE_MARKED <= 4)) and (itemtbl.item.Kind == "Crate" and not ldt)) then
                                    MOAT_ITEMS_CRATE_MARKED = 0
    ...
    for k, v in ipairs(MOAT_INV_CATS) do
        ...
        MOAT_CAT_BTN.DoClick = function(s)
            ...
            if (IsValid(MOAT_CAT_BAR) and MOAT_CAT_BAR.new_cat == s.CAT_NUM) then return end
            net.Start("MOAT_INV_CAT")
    ...
    M_SLOT_D.PaintOver = function(s, w, h)
        ...
        if (M_INV_DRAG.VGUI and M_INV_DRAG.VGUI.WModel and not string.EndsWith(M_INV_DRAG.VGUI.WModel, ".mdl")) then
            ...
            if (M_INV_DRAG.VGUI.Item and M_INV_DRAG.VGUI.Item.item and M_INV_DRAG.VGUI.Item.item.Clr) then
                cdn.DrawImage(M_INV_DRAG.VGUI.WModel, 0, 0, w, h, {
                    r = M_INV_DRAG.VGUI.Item.item.Clr[1],
                    g = M_INV_DRAG.VGUI.Item.item.Clr[2],
                    b = M_INV_DRAG.VGUI.Item.item.Clr[3],
                    a = 200
                })
        ...
        if (s.Item and s.Item.item) then
            ...
            if (m_Inventory[num].marked) then
                surface_SetDrawColor(132, 222, 2, 200)
                surface_DrawRect(0, 0, w, h)
            end
    ...
    function m_InventoryPanel(anim_time)
        ...
        function m_CreateInvSlot(num)
            ...
            m_DPanel.Paint = function(s, w, h)
                ...
                if ((M_INV_SP.Scroll > (slot_y + h)) or (slot_y > (M_INV_SP.Scroll + 480))) then return end
            ...
            m_DPanelIcon.SIcon.PaintOver = function(self, w, h)
                if (not M_INV_SLOT[num]) then return end
            ...
            m_DPanelBTN.OnMousePressed = function(s, key)
                ...
                if (key == MOUSE_LEFT and input.IsKeyDown(KEY_LCONTROL) and m_Inventory[num].c and m_Inventory[num].item.Kind == "Crate" and MOAT_ITEMS_DECON_MARKED <= 0) then
                    if (m_Inventory[num].l and m_Inventory[num].l == 1) then return end
                    if not table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup()) then return end
                    if MOAT_ITEMS_CRATE_MARKED > 5 then return end
                    moat_RemoveEditPositionPanel()

                    if (m_Inventory[num].marked) then
                        m_Inventory[num].marked = false
                        MOAT_ITEMS_CRATE_MARKED = math.Clamp(MOAT_ITEMS_CRATE_MARKED - 1, 0, 1000)
                    else
                        m_Inventory[num].marked = true
                        MOAT_ITEMS_CRATE_MARKED = math.Clamp(MOAT_ITEMS_CRATE_MARKED + 1, 0, 1000)
                        m_DrawCrateButton(MOAT_INV_BG)
                    end

                    if (input.IsKeyDown(KEY_LSHIFT)) then
                        if (MOAT_CRATE_ITEMS_START ~= 0) then
                            MOAT_CRATE_ITEMS_END = num

                            for i = MOAT_CRATE_ITEMS_START, MOAT_CRATE_ITEMS_END do
                                if ((m_Inventory[i].l and m_Inventory[i].l == 1) or not m_Inventory[i].c) then continue end

                                if (not m_Inventory[i].marked) then
                                    MOAT_ITEMS_CRATE_MARKED = MOAT_ITEMS_CRATE_MARKED + 1
                                end

                                m_Inventory[i].marked = true
                            end
                        end

                        if (MOAT_CRATE_ITEMS_START == 0) then
                            MOAT_CRATE_ITEMS_START = num
                        elseif (MOAT_CRATE_ITEMS_END ~= 0) then
                            MOAT_CRATE_ITEMS_START = 0
                            MOAT_CRATE_ITEMS_END = 0
                        end
                    end
                    ...
                    return
                elseif (key == MOUSE_LEFT and input.IsKeyDown(KEY_LCONTROL) and m_Inventory[num].c and m_Inventory[num].item.Kind ~= "Crate" and MOAT_ITEMS_CRATE_MARKED <= 0) then
                    if (m_Inventory[num].l and m_Inventory[num].l == 1) then return end
                    if not GetConVar("moat_deconhighends"):GetBool() and (m_Inventory[num].item and m_Inventory[num].item.Rarity and m_Inventory[num].item.Rarity > 5) then return end
                    moat_RemoveEditPositionPanel()
        ...
        function m_HandleLayoutSpacing(remove)
            if (remove) then
                if (IsValid(M_INV_L.Spacing)) then
                    M_INV_L.Spacing:Remove()
                end
        ...
        function m_CreateInventorySlots(noscroll, start, stop)
            if (not m_isUsingInv(true)) then return end
            start = start or 1
    ...
    function m_ChangeInventoryPanel(cat, trading, cb)
        ...
        if (cat == 3) then
            ...
            if (trading and IsValid(MOAT_TRADE_BG)) then
                ...
                MOAT_TRADE_BG:AlphaTo(0, anim_time, 0, function()
                    callback()
                end)
        ...
        if (cat ~= 0 and IsValid(M_INV_PNL_EXTND)) then
            M_INV_PNL_EXTND.Extended = false
        end

        if (cat == 0 or cat == -1 or cat == -2) then
            if (cat == -1) then
                MOAT_CAT_BAR.new_cat = 2
            end
    ...
    MOAT_INV_S.Paint = function(s, w, h)
        ...
        if (ITEM_HOVERED and ITEM_HOVERED.c) then
            ...
            if (ITEM_HOVERED.n) then end -- draw_xp_lvl = draw_xp_lvl + 15
            ...
            if (ITEM_HOVERED.item.NameEffect) then
                ...
                elseif (tfx == "threecolors") then
                    if (not ITEM_HOVERED.item.NameEffectMods[4]) then
                        ITEM_HOVERED.item.NameEffectMods[4] = 1
                    end

                    if (not ITEM_HOVERED.item.NameEffectMods[5]) then
                        ITEM_HOVERED.item.NameEffectMods[5] = RealTime()
                    end
                    ...
                    if (ITEM_HOVERED.item.NameEffectMods[5] <= RealTime()) then
                        ...
                        if (ITEM_HOVERED.item.NameEffectMods[4] > 3) then
                            ITEM_HOVERED.item.NameEffectMods[4] = 1
                        end
            ...
            if (ITEM_HOVERED.n) then end -- emoji.SimpleText("\"" .. ITEM_HOVERED.n:Replace("''", "'") .. "\"", "moat_ItemDesc", draw_name_x, draw_name_y + 21, Color(255, 128, 128))
            drawn_stats = 0
            ...
            elseif (ITEM_HOVERED.item and ITEM_HOVERED.item.Kind ~= "tier" and ITEM_HOVERED.item.Kind ~= "Unique" and ITEM_HOVERED.item.Kind ~= "Melee" and ITEM_HOVERED.item.Description) then
                ...
                if (ITEM_HOVERED.s and ITEM_HOVERED.item and ITEM_HOVERED.item.Stats) then
                    for i = 1, #item_desctbl do
                        if (not ITEM_HOVERED.item.Stats[i]) then continue end
        ...
        if (ITEM_HOVERED and ITEM_HOVERED.c) then
            ...
            if (ITEM_HOVERED.t) then
                ...
                for k, v in ipairs(ITEM_HOVERED.t) do
                    if (not ITEM_HOVERED.Talents[k] or not ITEM_HOVERED.Talents[k].Description) then continue end
                    local talent_desc2 = ITEM_HOVERED.Talents[k].Description
                    local talent_desctbl2 = string.Explode("^", talent_desc2)

                    for i = 1, #v.m do
                        if (not ITEM_HOVERED.Talents[k].Modifications or not ITEM_HOVERED.Talents[k].Modifications[i]) then continue end
            ...
            if ((ITEM_HOVERED.item and ITEM_HOVERED.item.Rarity or 0) == 0 and ITEM_HOVERED.item.ID and ITEM_HOVERED.item.ID ~= 7820 and ITEM_HOVERED.item.ID ~= 7821) then end -- panel_height = 100
            ...
            s:SetAlpha(255)
            if (moat_imagehack) then return end
    ...
    function m_InitializeTrade(ply_2, u_trade)
        if (ply_2 and u_trade and not IsValid(MOAT_TRADE_BG)) then
            ...
            MOAT_TRADE_AVA2.Think = function(s) end
            ...
            M_TRADE_IC_ENTRY:SetPaintBackground(false)
            ...
            M_TRADE_CHAT_ENTRY:SetPaintBackground(false)
...
function m_CreateItemMenu(num, ldt)
    ...
    function input.IsKeyDown(key)
        if moat_imagehack and (key == KEY_LCONTROL) then return true end
    ...
    M_INV_MENU2:AddOption("Share Screenshot", function()
        ...
        if MOAT_CACHED_PICS[itemtbl.c] then
            ...
            if MOAT_CACHED_PICS[itemtbl.c][1] == x then
                ...
                Derma_Message("Wowzie! Your item stat screenshot's Sharable URL has been copied to your clipboard :D\nTrade chat in Discord to start #buying @ #selling via https://discord.tera.gg", "Share Screenshot", "Deal (Close)")
                SetClipboardText(l)
        ...
        hook.Add("PostRender", "Gay render capture rules", function()
            ...
            HTTP({
                ...
                method = "post",
                headers = {
                    ["Authorization"] = "Client-ID 4f27af1a9057687"
                },
                success = function(_, c, _, _)
                    local album = util.JSONToTable(c)

                    if not album.success then
                        Derma_Message("Your upload was not successful (-2)! Please show this to sludge: " .. a, "Imgur failed", "Close")
                    ...
                    HTTP({
                        ...
                        success = function(_, b, _, _)
                            ...
                            if b.success then
                                ...
                                MOAT_CACHED_PICS[itemtbl.c] = {x, l}
                                --s
                            else
                                Derma_Message("Your upload was not successful! Please show this to sludge:\n" .. ob, "Imgur failed", "Close")
                            end
                ...
                failed = function(a)
                    Derma_Message("Your upload was not successful (-1)! Please show this to sludge: " .. a, "Imgur failed", "Close")
                    MOAT_UPLOADING = false
                end,
                parameters = {},
            })

            -- title = (item_name) .. " \\|| " .. LocalPlayer():Nick() .. " \\|| " .. GetServerName() or "tera.gg",
            -- description = (item_name) .. "\nOwned by " .. LocalPlayer():Nick() .. " (" .. LocalPlayer():SteamID() .. ") (https://steamcommunity.com/profiles/" .. LocalPlayer():SteamID64() .. ")\nShared via " .. (GetServerName() or "tera.gg") .. "\n\nTrade chat in Discord to start #buying @ #selling via https://discord.gg/xHv9XJVBXp"
            HoveringSlot = false
            m_HoveredSlot = nil
        end)
    ...
    if (itemtbl.n) then
        M_INV_MENU:AddOption("Remove Name Mutator", function() end):SetIcon("icon16/tag_blue_delete.png")
    end
    ...
    if (itemtbl.p) then
        ...
        M_INV_MENU:AddOption("Remove " .. p1txt, function() end):SetIcon("icon16/palette.png")
    end
    ...
    if (itemtbl.p2) then
        ...
        M_INV_MENU:AddOption("Remove " .. p2txt, function() end):SetIcon("icon16/paintcan.png")
    end
    ...
    if (table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup())) then
        multiplier = multiplier + .25
    end
...
function m_DrawTradeRequest(ply)
    ...
    M_REQ_A.DoClick = function()
        ...
        MOAT_REQ_BG:AlphaTo(0, 0.5, 0, function(anim, pnl)
            if (IsValid(pnl)) then
                pnl:Remove()
            end
        end)
    ...
    if (MT_TR and not MT_TR.Close) then return end
...
function m_DrawFoundItem(tbl, s_type, name)
    ...
    if (ITEM_HOVERED and ITEM_HOVERED.c) then
        ...
        if (ITEM_HOVERED.t) then
            ...
            for k, v in ipairs(ITEM_HOVERED.t) do
                if (not ITEM_HOVERED.Talents[k] or not ITEM_HOVERED.Talents[k].Modifications or not ITEM_HOVERED.Talents[k].Description) then continue end
                local talent_desc2 = ITEM_HOVERED.Talents[k].Description
                local talent_desctbl2 = string.Explode("^", talent_desc2)
    ...
    if (s_type == "inspect") then
        ...
        if (not att_cache) then return end
...
net.Receive("MOAT_ITEM_OBTAINED", function(len)
    ...
    if (last_played <= CurTime() - .5) then
        if (tbl.item and tbl.item.Rarity == 5) then
            cdn.PlayURL("https://tera.gg/ttt/drops/highend.mp3", .2)
        elseif (tbl.item and tbl.item.Name and string.EndsWith(tbl.item.Name, "Mutator")) then
            cdn.PlayURL("https://tera.gg/ttt/drops/mutator.mp3", .2)
        end
...
net.Receive("MOAT_INIT_USABLE", function()
    ...
    if (itemtbl.item.ItemCheck) then
        ...
        if (itemtbl.item.ID == 10 or itemtbl.item.ID == 13) and (GetRoundState() ~= 2 or player.GetCount() < 8) then
        ...
        if (itemtbl.item.ID == 705 and table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup())) then
            Derma_Message("You are already MVP or higher!", "Unusable at the moment", "Ok")
...
net.Receive("MOAT_MAX_SLOTS", function(len)
    ...
    if (max_slots > 350) then
        -- HTTP({
        -- 	url = "https://discord.tera.gg/api/webhooks/638353224796995584/r5ciN3MS-xit0iJokWb3Gd-iwkJ0kxw28JBtdFk45NhJnkXObF3O4P7qhWhO1YguO8pF",
        -- 	method = 'POST',
        -- 	headers = {
...
function m_DrawCrateButton()
    if (IsValid(MOAT_INV_MASS_CRATE)) then return end
    MOAT_INV_BG:SetSize(MOAT_INV_BG_W, MOAT_INV_BG_H + 40)
    MOAT_INV_MASS_CRATE = vgui.Create("DButton", MOAT_INV_BG)
    MOAT_INV_MASS_CRATE:SetPos(MOAT_INV_BG_W - 364 - 5, MOAT_INV_BG_H)
    MOAT_INV_MASS_CRATE:SetSize(364, 35)
    MOAT_INV_MASS_CRATE:SetText("")
    sfx.SoundEffects(MOAT_INV_MASS_CRATE)
    local hover_coloral = 1

    MOAT_INV_MASS_CRATE.Paint = function(s, w, h)
        surface_SetDrawColor(0, 0, 0, 255)
        surface_DrawRect(0, 0, w, h)
        surface_SetDrawColor(50, 50, 50, 100)
        surface_DrawOutlinedRect(0, 0, w, h)
        surface_SetDrawColor(133, 222, 2, 20 + hover_coloral / 5)
        surface_DrawRect(1, 1, w - 2, h - 2)
        surface_SetDrawColor(133, 222, 2, 20 + hover_coloral / 5)
        surface_SetMaterial(gradient_d)
        surface_DrawTexturedRect(1, 1, w - 2, h - 2)
        ...
        if (MOAT_ITEMS_CRATE_MARKED > 1) then
            the_s = "s"
        end

        m_DrawShadowedText(1, "Open " .. MOAT_ITEMS_CRATE_MARKED .. " Marked Item" .. the_s, "Trebuchet24", w / 2, h / 2, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end

    local btn_hovered = 1
    local btn_color_a = false

    MOAT_INV_MASS_CRATE.Think = function(s)
        if (MOAT_ITEMS_CRATE_MARKED < 1) then
            s:Remove()
            MOAT_INV_BG:SetSize(MOAT_INV_BG_W, MOAT_INV_BG_H)
    ...
    MOAT_INV_MASS_CRATE.DoClick = function()
        local items_marked = 0

        for i = 1, #m_Inventory do
            if (m_Inventory[i] and m_Inventory[i].marked) then
                items_marked = items_marked + 1
            end
        end

        if items_marked > 5 or MOAT_ITEMS_CRATE_MARKED > 5 then return end
        local items_sent = 0
        ...
        for i = 1, #m_Inventory do
            if (m_Inventory[i] and m_Inventory[i].marked) then
                ...
                if items_sent == MOAT_ITEMS_CRATE_MARKED then
                    chat.AddText(mat_info, Color(255, 255, 0), "You have opened ", Color(0, 255, 0), string.Comma(MOAT_ITEMS_CRATE_MARKED), Color(255, 255, 0), " Crates !")
                end
...
net.Receive("MOAT_DECON_NOTIFY", function()
    ...
    if (decon_last_played <= CurTime() - .5) then
        cdn.PlayURL("https://tera.gg/ttt/drops/decon_notify.mp3")
...
net.Receive("MOAT_DECON_MUTATOR", function()
    cdn.PlayURL("https://tera.gg/ttt/mutator.mp3")
end)
