...
function discordrpc.Init(callback)
    if not discordrpc.port then
        ...
        for port = 6463, 6473 do
            ...
            local failed = function(...) end -- do nothing
...
function discordrpc.GetActivity()
    ...
    activity = {
        details = player.GetCount() .. "/" .. game.MaxPlayers() .. " Players | " .. math.max(0, GetGlobal("ttt_rounds_left")) .. " Rounds Left | " .. round,
...
discordrpc.clientID = "139152491496472576"
discordrpc.BotclientID = "802257856308838430"
discordrpc.state = ""
...
--[[
local config_discordurl = "https://discord.tera.gg" --Discord URL
local config_discord_close_delay = 4 -- four seconds
...
timer.Simple(5, function()
    ...
    html:Dock(FILL)
    html:KillFocus()
    html:OpenURL(config_discordurl)
    frame:Hide()

    timer.Simple(config_discord_close_delay, function()
        frame:Close()
    end)
end)]]