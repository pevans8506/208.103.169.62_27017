...
local cached_toxic = {}
local cached_err = {}
local api_key = "AIzaSyDsFIInpcq80RbwN0Gk7-5hVpL5_s68rdc"
        ...
        success = function (code,body)
            ...
            if code == 200 then
                ...
                for k,v in pairs(requestedAttributes) do
					...
					s = s .. ("[▰](https://tera.gg)"):rep(plus) .. ("▱"):rep(20 - plus)
                    s = s .. "\n"
				...
				http.Fetch("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=1A1B81A379F749EFFA22927033E0ED34&steamids=" .. ply:SteamID64() .. "&format=json", function(body, size, headers, code)
					...
					discord.Embed("Toxic TTT Loggers", {
						color = 0,
						description = message .. '\n' .. s .. '\nSaid on ' .. GetServerName() .. ' (' .. GetServerURL() .. ')',
						author = {
						...
						thumbnail = {
							url = "https://tera.gg/ttt/tera-i.png"
						},
...
net.Receive("moat.verify", function(_, pl)
    ...
    if (dev_server or pl:IsUserGroup("director")) then return end
