...
MOAT_DONATE.BackgroundURL = "https://tera.gg/ttt/misc/bg.png"
...
MOAT_DONATE.Packages = {
    ...
    {"Free Rewards", Material("icon16/star.png"), 0, 0, ""},
    {
        "Permanent VIP", Material("icon16/user.png"), 2, 1500, "", {
            {Color(255, 255, 0), "17,000", "Total Inventory Credits", ""},
    ...
    {
        "Permanent MVP", Material("icon16/user.png"), 3, 3000, "", {
            {Color(255, 255, 0), "32,000", "Total Inventory Credits", ""},
            {Color(255, 255, 0), "", "Earn 75% more IC when Deconstructing Items	", ""},
            {Color(255, 255, 255), "MVP", "Rank In-Game & Forums", ""},
            {Color(255, 255, 255), "", "Votekick Starting Access (Revokable)", ""},
            {Color(255, 255, 255), "", "Access to closed beta(s)", ""},
            {Color(255, 255, 255), "", "Access to beta feature(s)", ""},
            {Color(255, 0, 0), "", "+ Whatever is Added to MVP in the Future!", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "4,000 IC", Material("icon16/coins.png"), 4, 400, "", {
            {Color(255, 255, 0), "4,000", "Total Inventory Credits", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "10,000 IC", Material("icon16/coins.png"), 5, 1000, "", {
            {Color(255, 255, 0), "10,000", "Total Inventory Credits", ""},
            {Color(255, 0, 255), "15", "Random Crates from the Market", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "25,000 IC", Material("icon16/coins.png"), 6, 2500, "3,000 Bonus", {
            {Color(255, 255, 0), "28,000", "Total Inventory Credits", ""},
            {Color(0, 125, 255), "1", "Event Credit", ""},
            {Color(255, 205, 0), "1", "Ascended Talent Mutator", ""},
            {Color(255, 0, 0), "[LIMITED TIME]", "7 Easter Baskets!", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "40,000 IC", Material("icon16/coins.png"), 7, 4000, "10,000 Bonus", {
            ...
            {Color(255, 0, 0), "[LIMITED TIME]", "11 Easter Baskets!", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "100,000 IC", Material("icon16/coins.png"), 8, 10000, "35,000 Bonus", {
            {Color(255, 255, 0), "135,000", "Total Inventory Credits", ""},
            {Color(0, 125, 255), "7", "Event Credits", ""},
            {Color(255, 125, 0), "2", "Name Mutators", ""},
            {Color(255, 205, 0), "1", "Ascended Talent Mutator", ""},
            {Color(0, 255, 0), "1", "Cosmic Talent Mutator", ""},
            {Color(255, 0, 0), "[LIMITED TIME]", "20 Easter Baskets!", ""},
            {Color(255, 0, 125), "", "", ""}
        }
    },
    {
        "Dola Drop Event", Material("icon16/heart.png"), 9, 3750, "", {
            {Color(0, 255, 0), "", "Server Dola Effect Drop", ""},
            {Color(255, 125, 0), "", "Every player will receive the Dola Effect Item", ""},
            {Color(255, 0, 125), "", "Drops for everybody on the server!", ""},
            {Color(255, 255, 0), "", "(this is tradable)", ""}
        }
    },
    {
        "+300% XP Event", Material("icon16/heart.png"), 10, 250, "", {
            {Color(0, 255, 0), "", "Server +300% XP Event", ""},
            {Color(255, 125, 0), "", "Every player earns +300% more Boosted XP", ""},
            {Color(255, 0, 125), "", "Boosts XP for 20 rounds!", ""},
            {Color(255, 255, 0), "", "(this is stackable)", ""}
        }
    },
    {
        "Vape Drop Event", Material("icon16/heart.png"), 11, 4420, "", {
            {Color(0, 255, 0), "", "Server Random Vape Drop", ""},
            {Color(255, 125, 0), "", "Every player will receive a Random Vape Item", ""},
            {Color(255, 0, 125), "", "Drops for everybody on the server!", ""},
            {Color(255, 255, 0), "", "(this is tradable)", ""}
        }
    }
...
--{"30 Day Star Player", Material("icon16/user_green.png"), 2000, "Stuff"}, ??? --{"1 Year Star Player", Material("icon16/user_red.png"), 20000, "Stuff"}, ??? -- MAke sure you don't take event out of name cause it's used to check if its a map event
...
function MOAT_DONATE:DrawRewardsInfo(pnl, pkg, clr)
    ...
    --cdn.DrawImage("https://cdn.notfound.tech/cWeit2ZL5WhFze49xX7jV76mFvOG.png", (w/2) - (235/2), 55, 254, 235)
    ...
    function steam:Paint(w, h)
        ...
        surface.DrawTexturedRect(0, 0, w, 45)
        surface.DrawOutlinedRect(0, 0, w, 45)
        cdn.DrawImageRotated("https://tera.gg/ttt/misc/steam.png", 10, 4, 35, 35, nil, math.sin(CurTime()) * 15, true)
    ...
    function steam:Paint(w, h)
        ...
        surface.DrawTexturedRect(0, 0, w, 45)
        surface.DrawOutlinedRect(0, 0, w, 45)
        cdn.DrawImageRotated("https://tera.gg/assets/img/mg-square2.png", 10, 4, 35, 35, nil, math.sin(CurTime() + 10) * 15, true)
    ...
    if not hastag then
        lb:SetText("NOTE: You can only get daily rewards by adding tera.gg to your name! Capitalization doesn't matter")
    else
        lb:SetText("You have tera.gg in your name and are currently receiving daily rewards!")
    end
    ...
    function collect:DoClick()
        if MOAT_REWARDS.IC < 1 then return end
...
function MOAT_DONATE:RebuildSelection(num)
    ...
    pnl.Paint = function(s, w, h)
        ...
        if (pkg[6]) then
            ...
            elseif (pkg[3] == 3 and table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup())) then
                draw.SimpleText("This Tradeable MVP Token Includes:", "moat_Derma7", w / 2, 85, Color(0, 255, 255), TEXT_ALIGN_CENTER)
            else
    ...
    pnl.r.DoClick = function(s)
        ...
        if (pkg[3] == 2 and LocalPlayer():GetUserGroup() ~= "user") then
            ...
            -- net.SendToServer()
        elseif (pkg[3] == 3 and table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup())) then
            Derma_Query("Are you sure you want to purchase a MVP Token that you cannot redeem?\n(Can be traded/gifted to other players to give them MVP)", "Are you sure?", "Yes", function()
                if (IsValid(self.bg)) then
                    self.bg:Remove()
                end
        ...
        else -- m_ClearInventory() -- net.Start("MOAT_SEND_INV_ITEM") -- net.SendToServer()
            if (IsValid(self.bg)) then
                self.bg:Remove()
            end
...
function MOAT_DONATE:OpenWindow()
    ...
    if table.HasValue(MOAT_MVP, LocalPlayer():GetUserGroup()) then
        MOAT_DONATE.Packages[4][1] = "MVP Token"
    end

    if LocalPlayer():GetUserGroup() == "vip" then
        MOAT_DONATE.Packages[4][4] = 1500
    end
    ...
    self.bg.Paint = function(s, w, h)
        ...
        local w2, h2 = draw.SimpleText("Tera", "moat_GambleTitle", 4, 0, Color(0, 198, 255))
    ...
    g.DoClick = function(s)
        gui.OpenURL("https://tera.gg/store/")
    end
