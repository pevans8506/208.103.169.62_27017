...
MOAT_DONATE.Packages = {
    ...
    [3] = {
        3000, function(pl, discount)
            if table.HasValue(MOAT_MVP, pl:GetUserGroup()) then
                pl:m_DropInventoryItem("MVP Token")
                net.Start("D3A.Chat2")
                net.WriteBool(false)

                if discount then
                    net.WriteTable({"Successfully redeemed ", Color(0, 255, 0), "1,500 ", Color(255, 255, 255), "Support Credits for the MVP Token Package!"})
                else
                    net.WriteTable({"Successfully redeemed ", Color(0, 255, 0), "3,000 ", Color(255, 255, 255), "Support Credits for the MVP Token Package!"})
                end
    ...
    [6] = {
        2500, function(pl)
            ...
            for i = 1, 7 do
                pl:m_DropInventoryItem("Easter Basket", "hide_chat_obtained", false, false)
            end
...
function MOAT_DONATE.Purchase(l, pl)
    ...
    if id == 3 and pl:GetUserGroup() == "vip" then
        pl:TakeSC(pkg[1] / 2, function(new)
            if (sc > new) then
                MoatLog(pl:SteamID() .. " purchased package #" .. id .. " for " .. pkg[1] / 2 .. " Support Credits")
                pkg[2](pl, true)
            end
        end)
    elseif (sc and tonumber(sc) >= pkg[1] and not pl.StoreBusy) then
...
local function namerewards()
    ...
    local function nametag(ply)
        return ply:Nick():lower():match("tera%.gg")
    end
    ...
    net.Receive("NameRewards.Collect", function(l, ply)
        ...
        ply.pendingname = nil
        q:start()
        ply:SendLua("chat.AddText('You have collected your points! Thanks for supporting TERA TTT <3')")
    end)
end
