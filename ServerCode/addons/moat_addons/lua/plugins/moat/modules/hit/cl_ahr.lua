...
moat_Settings[1] = {
    "Enable alternative hit registration?", {"Yes", "No"},
    "moat_alt_hitreg"
}

moat_Settings[2] = {
    "Enable hitmarkers?", {"Yes", "No"},
    "moat_hitmarkers"
}

moat_Settings[3] = {
    "Enable hitmarker sound?", {"Yes", "No"},
    "moat_hitmarker_sound"
}

moat_Settings[4] = {
    "Hitmarker Size", {"Small", "Normal", "Large"},
    "moat_hitmarker_size"
}

moat_Settings[5] = {
    "Hitmarker Color", {"moat_hitmarker_color_r", "moat_hitmarker_color_g", "moat_hitmarker_color_b", "moat_hitmarker_color_a"}
}
...
local function moatHitRegSettings()
    ...
    MOAT_BG.Paint = function(s, w, h)
        ...
        draw.SimpleTextOutlined("Alternative Hit Registration Settings (" .. ping .. "ms)", "moat_LabelFont", 10, 12, Color(255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 20))
    end
...
net.Receive("moat_damage_number", function()
    ...
    if (dmg <= 0 or not pos) then return end
...
hook.Add("EntityFireBullets", "‍a", function(e, t)
    ...
    if (not IsValid(t.Attacker) or t.Attacker ~= LocalPlayer() or not c:GetBool()) then return end
    local cb = t.Callback
