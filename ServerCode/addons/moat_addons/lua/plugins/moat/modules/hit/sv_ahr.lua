...
function ENTITY:HitRegCheck()
    --MAX_Ping
    return self:IsPlayer() and self:GetInfoNum("moat_alt_hitreg", 1) == 1 and self:Ping() < 300
end
...
function SHR:WeHit(shooter, ent, wpn, eye, localposx, localposy, localposz, dmgfrc, hg, fire_num, shot_num)
    if (not IsValid(wpn) or not IsValid(ent) or not IsValid(shooter)) then return end
    if (shooter:GetObserverMode() ~= OBS_MODE_NONE or wpn:GetOwner() ~= shooter or not IsValid(ent) or (ent:IsPlayer() and ent:GetObserverMode() ~= OBS_MODE_NONE)) then return end
    local localpos = Vector(localposx, localposy, localposz)
    local mins, maxs = ent:OBBMins(), ent:OBBMaxs()
    if (localpos:DistToSqr((mins + maxs) / 2) > mins:DistToSqr(maxs)) then return end
    ...
    if (ent:IsPlayer() and (hook.Run("ScalePlayerDamage", ent, hg, dmginfo) or hook.Run("PlayerShouldTakeDamage", ent, shooter) == false)) then return end

    if (ent:IsNPC()) then
        hook.Run("ScaleNPCDamage", ent, hg, dmginfo)
    end
...
local function hiteffects(targ, dmg)
    if (GetRoundState() == ROUND_PREP or not dmg) then return false end --[[or dmg:GetDamage() <= 0]]
    local att = dmg:GetAttacker()
    if (not IsValid(targ) or not IsValid(att) or not att:IsPlayer()) then return false end
    local apache = (IsValid(MOAT_APACHE_ENT) and MOAT_APACHE_ENT == targ) or (IsValid(MOAT_BOSS_CUR_PLY) and MOAT_BOSS_CUR_PLY == targ)
    if (not targ:IsPlayer() and not apache) then return false end
...
local function takedamage(gm, targ, dmg)
    ...
    if (not hiteffects(targ, dmg)) then return end
...
local function entityfirebullets(gm, e, t)
    ...
    if (e:HitRegCheck()) then
        ...
        t.Callback = function(a, tr, d)
            ...
            num = num + 1
            if (cb) then return cb(a, tr, d) end
        end
    end
...
net.ReceiveNoLimit("shr", function(_, pl)
    if (not pl:HitRegCheck() or pl:GetObserverMode() ~= OBS_MODE_NONE or net.ReadUInt(32) ~= SHR_Val) then return end
    SHR:WeHit(pl, net.ReadEntity(), net.ReadEntity(), net.ReadVector(), net.ReadFloat(), net.ReadFloat(), net.ReadFloat(), net.ReadVector(), net.ReadUInt(4), net.ReadUInt(32), net.ReadUInt(8))
end)
