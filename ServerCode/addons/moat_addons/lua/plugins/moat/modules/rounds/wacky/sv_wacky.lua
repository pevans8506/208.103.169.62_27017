...
moat_random.register("AWP","Everyone spawns with AWP's!",{
    ["NOW"] = function()
        for k, v in ipairs(player.GetAll()) do
			if (v:Team() == TEAM_SPEC) then continue end

			v:Give("weapon_ttt_awp")
			v:SelectWeapon("weapon_ttt_awp")
        end
    end,
    ["TTTBeginRound"] = function()
        for k, v in ipairs(player.GetAll()) do
			...
			timer.Simple(spn and 2 or 0, function()
				if (not IsValid(v) or v:Team() == TEAM_SPEC) then return end
				if (v:HasWeapon("weapon_ttt_awp") and not spn) then return end
...
moat_random.register("High HP", "Base health is 250 HP for this round!", {
    ["NOW"] = function()
		for k, v in pairs(player.GetAll()) do
			if ((v.MaxHealth and v.MaxHealth > 250) or v:GetMaxHealth() > 250 or v:Health() > 250) then
				continue
			end
		...
		net.Start("RandomRound")
    		...
    		net.WriteString("Base health is 250 HP for this round!")
    	net.Broadcast()
    end,
    ["TTTBeginRound"] = function()
        for k, v in pairs(player.GetAll()) do
...
moat_random.register("Small round", "Everyone is 1/10th the size!", {
    ["NOW"] = function()
		for k, v in pairs(player.GetAll()) do
		...
		net.Start("RandomRound")
    		net.WriteString("Small round")
    		net.WriteString("Everyone is 1/10th the size!")
    	net.Broadcast()
    end,
    ["TTTBeginRound"] = function()
        for k, v in pairs(player.GetAll()) do
