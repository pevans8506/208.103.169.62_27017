...
local rewardsapi = "https://tera.gg/api/steam/rewards/"
local groupapi = "https://tera.gg/api/steam/group/"
...
local function GiveRewards()
    ...
    btn.DoClick = function(s)
        ...
        gui.OpenURL("https://steamcommunity.com/groups/defcongamingcommunity")
    end
...
local function CheckRewards()
    if (cookie.GetNumber("tera_group_rewards", 0) == 1) then return end
    local id = LocalPlayer():SteamID64()
    if (not id) then return end
    ...
    http.Fetch(rewardsapi .. id, function(b)
        ...
        elseif (b.value == 1) then
            cookie.Set("tera_group_rewards", 1)
        end
    end, function()
        timer.Simple(30, function()
            CheckRewards()
