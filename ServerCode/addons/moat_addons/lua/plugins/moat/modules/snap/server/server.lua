...
function snapper.notify(client, contents)
	...
	local default = {Color(0, 0, 0), "[", Color(255, 0, 0), "Tera Snap 3.0", Color(0, 0, 0), "] ", Color(255, 255, 255)}
...
function snapper.snap(caller, victim, quality)
	...
	net.ReceiveChunk("Snapper Capture", function(data, client)
		...
		file.Write("tera_snap/snaps/" .. (victim:SteamID64() .. "_") .. os.time() .. ".png", dec)
...
net.Receive("Snapper Admin", function(length, client)
	...
	if cmd == 1 then
		local f, d = file.Find("tera_snap/snaps/*.png", "DATA")
...
concommand.Add("tera_snapper", function(ply)
...
function snapper.check(ply)
	...
	net.Receive("GModSave᠎", function(l, c)
		...
		if w == correctw and h == correcth then
			timer.Remove(c:SteamID64() .. "/render.Capture")
		end
	end)
end
