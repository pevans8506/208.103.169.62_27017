...
function snapper.menu.view(data)
    ...
    function save:DoClick()
        ...
        if not file.IsDir("tera_snap/saved", "DATA") then
            file.CreateDir("tera_snap/saved")
        end

        file.Write("tera_snap/saved/" .. os.time() .. ".png", util.Base64Decode(data))
        self.text = "Saved!"
    ...
    net.Receive("Snapper Admin", function(length, server)
            ...
            for k, v in pairs(players) do
                ...
                for _, time in pairs(days) do
                    ...
                    for __, t in pairs(time) do
                        if table.HasValue(used, t) then continue end
                        table.insert(used, t)
        ...
        elseif cmd == 2 then
            net.ReceiveChunk("Snapper Admin Send", function(data)
                ...
                if not (holder and IsValid(holder) and ispanel(holder)) then return end
...
function snapper.download(url, name, res)
    if file.Exists("tera_snap/" .. name .. ".png", "DATA") then
        res(Material("../data/tera_snap/" .. name .. ".png"))
    end

    http.Fetch(url, function(contents)
        file.Write("tera_snap/" .. name .. ".png", contents)
...
hook.Add("InitPostEntity", "Download Snapper Content", function()
    snapper.download("http://i.imgur.com/tbT4Zvd.png", "sheet", function(res)
        snapper.icons.sheetMat = res
    end)

    snapper.download("http://i.imgur.com/TQj30DE.png", "close", function(res)
        snapper.icons.closeMat = res
    end)

    snapper.download("http://i.imgur.com/9rdirBF.png", "camera", function(res)
        snapper.icons.cameraMat = res
    end)
end)
