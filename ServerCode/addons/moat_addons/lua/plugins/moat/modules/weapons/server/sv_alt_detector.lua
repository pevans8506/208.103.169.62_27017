...
local ban_reasons = {"[Automated] We don't allow family shared accounts on our servers, sorry!", "[Automated] We don't allow people that haven't setup their profile on our servers, sorry! Make an unban request on the forums after you setup your profile to be unbanned."}
...
local function steam_api(pl)
    ...
    http.Fetch(string.format("http://api.steampowered.com/IPlayerService/IsPlayingSharedGame/v0001/?key=%s&format=json&steamid=%s&appid_playing=4000", "1A1B81A379F749EFFA22927033E0ED34", id), function(body)
        ...
        ServerLog("Steam API Fail: " .. err .. "\n")
    end)
    --[[if (cn > 0) then return end

	http.Fetch(string.format("http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=%s&format=json&steamids=%s", "1A1B81A379F749EFFA22927033E0ED34", id),
		function(body)
...
local function moat_alts()
    ...
    net.Receive("MOAT_INIT_WEPS", function(_, pl)
        if (pl.AltsInitiated) then return end
