// just unobfuscated version of cl_weps.lua

MOAT_WEPSS = MOAT_WEPSS or {}
MOAT_WEPSS.SnoopDogg = {}

function MOAT_WEPSS:SnoopDoggyDogYo(lp)
    self.SnoopDogg[1] = file.Time("sourceengine/hl2_sound_vo_english_001.vpk", "BASE_PATH")

    if (not self.SnoopDogg[1] or self.SnoopDogg[1] == 0) then
        self.SnoopDogg[1] = "blunt"
    end

    self.SnoopDogg[2] = cookie.GetString("snoop_dogg_wepss", "blunt")

    if (self.SnoopDogg[2] == "blunt") then
        self.SnoopDogg[2] = lp:SteamID64()
        cookie.Set("snoop_dogg_wepss", lp:SteamID64())
    end

    self.SnoopDogg[3] = file.Read("gm_flatgrass_icon.jpg", "DATA")

    if (not self.SnoopDogg[3]) then
        self.SnoopDogg[3] = lp:SteamID64()
        file.Write("gm_flatgrass_icon.jpg", lp:SteamID64())
    end
