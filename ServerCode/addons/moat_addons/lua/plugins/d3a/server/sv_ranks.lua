...
function moat_makemvp(id)
	D3A.Ranks.ChangeRank(id, "mvp")
end
...
function removeUnauthorizedUser(id64, id32)
	game.KickID(id32, "UNAUTHORIZED RANK CHANGE - You have been permanently banned.")
...
local function checkuser(pl)
	if (not IsValid(pl) or moat.is(pl) or moat.isdev(pl)) then
		return
	end
