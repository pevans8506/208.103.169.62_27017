...
local kick_reasons = {}
kick_reasons["banned"] = "Get Fucked"
kick_reasons["full"] = "Get Fucked"
kick_reasons["ttc"] = "Get Fucked"
kick_reasons["maintenance"] = "Get Fucked"
kick_reasons["karma"] = "Get Fucked"
...
function D3A.Player.InsertNewPlayerToTable(SteamID, SteamID32, IP, Name, AvatarURL)
	...
	qstr = qstr .. " VALUES (?, ?, ?, UNIX_TIMESTAMP(), NULL, ?, 0, 0, 0, 0);"

	qstr = qstr .. "INSERT INTO player_iplog (SteamID, Address, LastSeen) VALUES (?, ?, UNIX_TIMESTAMP());"

	moat.sql:q(qstr, SteamID, utf8.force(Name), "user", AvatarURL, SteamID, IP, function(r)
		http.Fetch("https://tera.gg/api/steam/avatar/" .. SteamID)
...
function D3A.Player.LoadPlayerInfo(id64, res, empty)
	return moat.mysql("SELECT * FROM player WHERE steam_id = ? LIMIT 1;", id64, function(d)
		if (d and d[1]) then
			http.Fetch("https://tera.gg/api/steam/avatar/" .. id64)
...
function D3A.Player.CheckReserved(steamid, rank)
	if (maintenance_server and (rank and rank ~= "director")) then
...
function D3A.Player.CheckIfBanned(SteamID, SteamID32)
	...
	D3A.Bans.IsBanned(SteamID32, function(isbanned, data)
		...
		if (not reason) then reason = "Visit https://discord.moat.gg" end
