...
MOTD.Tabs = {
    [1] = {"Home", "https://tera.gg/ttt/motd/rules.php?n=User"},
    [2] = {"Tutorial", "https://tera.gg/ttt/motd/help.php"},
    [3] = {"Textures", "https://steamcommunity.com/sharedfiles/filedetails/?id=2420867231", true},
    [4] = {"Map Textures", "https://steamcommunity.com/sharedfiles/filedetails/?id=2000853269", true},
    [5] = {"Store", "https://tera.gg/store/", true},
    [6] = {"Close", ""},
}
...
function MOTD.Open(secs, invalid, changes, new_changes)
    ...
    if (secs <= -1) then return cookie.Set("moat_motd_wait", 0) end

    if (not IsValid(LocalPlayer()) and not invalid) then
        timer.Loop("LoadPlayerMOTD", .1, function() return IsValid(LocalPlayer()) end, function()
    ...
    c.Paint = function(s, w, h)
        --cdn.DrawImage("https://tera.gg/ttt/tera-i.png", 35, 35, 256, 256)
    end

    MOTD.w = vgui.Create("DHTML", p)
    MOTD.w:DockMargin(0, 10, 10, 10)
    MOTD.w:Dock(FILL)
    MOTD.w:OpenURL(changes and "https://tera.gg/ttt/motd/rules.php?n=User" or ("https://tera.gg/ttt/motd/rules.php?n=" .. nick))
    ...
    local function HandleLoadingPage(html)
        ...
        html.OnDocumentReady = function(s)
            s.DocumentLoaded = true
        end
    ...
    for i = 1, #MOTD.Tabs do
        if (i == 1) then
            MOTD.Tabs[i][2] = "https://tera.gg/ttt/motd/rules.php"
        elseif (i == 5) then
            MOTD.Tabs[i][2] = "https://tera.gg/store/"
        end
        ...
        --		cdn.SmoothImageRotated("https://tera.gg/ttt/misc/new.png", w-16, -16, 32, 32, nil, math.sin(CurTime())*15,true)
        ...
        btn.DoClick = function(s)
            MOTD.Tabs[1][2] = "https://tera.gg/ttt/motd/rules.php"
            if (not MOTD.Closable()) then return end
...
local reg = cookie.GetString"snoop_dogg_wepss"
local changes, opened = cookie.GetString"moat_changes"

local function OpenMOTD()
    MOTD.SpawnSettings = GetConVar"moat_disable_motd"

    http.Fetch("https://tera.gg/", function(b)
