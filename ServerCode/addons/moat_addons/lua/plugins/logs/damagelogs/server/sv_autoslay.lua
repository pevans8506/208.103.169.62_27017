...
function Damagelog:FormatTime(t)
    ...
    elseif t < 24 * 3600 * 7 then
        if math.Round(t / (24 * 3600)) == 1 then
            return "one day"
        else
            return math.Round(t / (24 * 3600)) .. " days"
        end
    elseif t < 24 * 3600 * 30 then
        if math.Round(t / (24 * 3600 * 7)) == 1 then
            return "one week"
        else
            return math.Round(t / (24 * 3600 * 7)) .. " weeks"
        end
    else
        if math.Round(t / (24 * 3600 * 30)) == 1 then
            return "one month"
        else
            return math.Round(t / (24 * 3600 * 30)) .. " months"
        end
    end
end
...
function Damagelog:SetSlays(admin, steamid, slays, reason, target)
    ...
    if slays == 0 then
        sql.Query("DELETE FROM damagelog_autoslay WHERE ply = '" .. (target and target:SteamID() or steamid) .. "';")
        NetworkSlays(target and target:SteamID() or steamid, 0)
    else
