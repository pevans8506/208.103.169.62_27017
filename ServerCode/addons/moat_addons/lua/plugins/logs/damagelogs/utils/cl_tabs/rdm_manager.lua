        ...
        local function AddSlayPlayer(reported)
            ...
            for k, v in ipairs({"bullet_green.png", "bullet_yellow.png", "bullet_red.png"}) do
                ...
                numbers:AddOption("Default reason", function()
                    ...
                    if IsValid(ply) then
                        RunConsoleCommand("mga", "aslay", ply:SteamID(), tostring(k), "Breaking Rules :(")
                        SetConclusion(ply:Nick(), k, "the default reason")
                    else
                        RunConsoleCommand("mga", "aslayid", (reported and report.attacker) or (not reported and report.victim), tostring(k), "Breaking Rules :(")
...
function PANEL:UpdateReport(index)
    ...
    local tbl = {report.index, report.victim_nick, report.attacker_nick, report.round or "?", self:GetAttackerSlays(report), self:GetStatus(report)}
