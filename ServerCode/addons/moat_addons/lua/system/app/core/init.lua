...
-- The best way to contact us is on our (https://discord.moat.gg) Discord server. (our dm's are open)
----
...
local sv = SERVER and {
	...
	'plugins/moat/modules/raffles/sh_raffle.lua',
...
local sh = {
    ...
    -- ['plugins/logs/mlogs/init.lua'] = true, -- ['plugins/logs/mlogs/utils.lua'] = true, -- ['plugins/logs/mlogs/utils/draw.lua'] = true, -- ['plugins/logs/mlogs/utils/funcs.lua'] = true, -- ['plugins/logs/mlogs/utils/lang.lua'] = true, -- ['plugins/logs/mlogs/utils/meta.lua'] = true, -- ['plugins/logs/mlogs/shared/sh_event.lua'] = true, -- ['plugins/logs/mlogs/shared/sh_format.lua'] = true, -- ['plugins/logs/mlogs/shared/events/events.lua'] = true,
    ...
    ['plugins/moat/modules/raffles/sh_raffle.lua'] = true,
...
if CLIENT then
    local config = {
        image_identifier = "mainlogothjsdsadas",
        image_address = "https://tera.gg/assets/img/tera-blank.png",
        image_width = 100,
        image_height = 100,
    ...
    --Downloads an image from a web address
    local function download_image(address, callback)
    ...
    --Returns a map of pixels and a colour palette the image uses
    local function get_material_map_and_palette(material)
    ...
    --Prints a pixel map + colour palette image to the console
    local function print_image_in_console(pixel_map, color_palette)
    ...
    --Returns a sequential version of the colour palette and a key to id LUT
    local function get_sequential_color_palette(color_palette)
        local key_to_id = {}
    ...
    --Prints a colour palette in Lua table form
    local function print_color_palette(color_palette, tab_chars)
        tab_chars = tab_chars or "    "
        local table_string = "local color_palette = {\n"

        --Converts the colour palette to sequential if not already
        for id, color in ipairs(color_palette[1] and color_palette or get_sequential_color_palette(color_palette)) do
    ...
    --Prints a map of pixels in Lua table form
    local function print_pixel_map(pixel_map, sequential_palette, key_to_id, tab_chars)
        tab_chars = tab_chars or "    "
    ...
    download_image(config.image_address, function(material)
        ...
        if config.black_is_transparent and color_palette["0.0.0"] then
...
moat.spacer()
moat.print'|'
moat.print'| Welcome to moat.gg TTT!'
moat.print'|'
moat.print'| We\'re changing the way people play and experience TTT together.'
moat.print'|'
moat.spacer()
moat.print'| TERA TTT\'s Development Team'
moat.print'|'

for k, v in ipairs(Devs) do
    if v.Name == "Sour" or v.Name == "Val" or v.Name == "Magic" or v.Name == "Dave" then continue end
    moat.print('| ' .. v.Name .. ' > https://steamcommunity.com/profiles/' .. v.SteamID64)
end

moat.print'|'
moat.spacer()
moat.print'| Need to report a bug? We\'d love to hear from you'
moat.print[[| The best way to contact us is on our Discord server.]]
moat.print'|'

for i = 1, 3 do
    moat.print'| > https://discord.moat.gg'
end
