...
function cdn.Fetch(key, folder, ext, cache, write, callback)
    ...
    elseif (file.Exists('tera_cache' .. (ext == '.txt' and '/2/' or '/1/') .. hashy .. ext, "DATA")) then
        object = 'tera_cache' .. (ext == '.txt' and '/2/' or '/1/') .. hashy .. ext
        cdn.Cache[key] = cache and cache(object) or ("data/" .. object)
...
if (SERVER) then
    ...
    function cdn.Sound(key, cb)
        if (cdn.Cache[key] == nil) then return cdn.Fetch(key, "sound", ".txt", function(object) return "data/" .. object end, function(object, data) return file.Write(object, data) end, cb) end
    ...
    function cdn.Image(key, cb, params)
        if not IsValid(cdn.Cache[key]) then return cdn.Fetch(key, "materials", key:match"(%.[^%.]+)$", function(object) return (not key:match"vtf$") and Material("../data/" .. object, (type(cb) == "string") and cb or params) or "../data/" .. object end, function(object, data) return file.Write(object, data) end, cb) end
    ...
    function cdn.Texture(key, cb, params)
        if (cdn.Cache[key] == nil) then return cdn.Fetch(key, "materials", ".vtf", function(object) return "../data/" .. object end, function(object, data) return file.Write(object, data) end, cb) end
    ...
    function cdn.TextureMaterial(key, cb, params)
        ...
        if (cdn.Cache[key] == nil) then
            ...
            return cdn.Fetch(key, "materials", ".vtf", function(object)
                ...
                return new_mat
            end, function(object, data) return file.Write(object, data) end, cb)
        end
