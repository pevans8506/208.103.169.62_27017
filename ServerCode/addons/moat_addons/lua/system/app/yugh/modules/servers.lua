...
Server = Server or {
    Name = GetHostName():match("- ([%w%s#]*)") or GetHostName() or "Tera TTT" or "Dev",
    ID = false,
    URL = false,
    Index = 200,
    ConnectURL = false,
...
function Server.BuildAddress()
    ...
    if (not hostip or not port) then return end
    local ipt, ip = {}
...
-- TTT Minecraft #2
----
GetServerName = function() return Server.Name or GetHostName() end
...
----
GetServerURL = function() return Server.ConnectURL or (Servers.SteamURL .. GetServerIP()) end

function servers.Get(str)
    if (not str) then return servers.Roster end
